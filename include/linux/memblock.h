 ...
 */
struct memblock_type {  //内存类型
	unsigned long cnt;  //当前内存块中内存区域的数量
	unsigned long max;  //所有内存区域的大小
	phys_addr_t total_size; //内存区域的已分配数组的尺寸
	struct memblock_region *regions;    //指向 `memblock_region` 结构体数据的指针的域
	char *name;
};

/**
 * struct memblock - memblock allocator metadata    它描述了一块通用的数据块
...
#define MEMBLOCK_ALLOC_ANYWHERE	(~(phys_addr_t)0) //=0xffffffffffffffff
