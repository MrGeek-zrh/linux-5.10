 ...
 */
/**
 * struct edac_raw_error_desc - EDAC错误详细描述结构体
 * @location: 错误发生的位置信息
 * @label: 受影响的DIMM标签
 * @grain: 错误报告的最小粒度(字节)
 * @error_count: 同类型错误的发生次数
 * @type: 错误类型(可纠正/不可纠正/延迟/致命) 
 * @top_layer: 错误的顶层(通常是channel)
 * @mid_layer: 错误的中层(通常是DIMM)
 * @low_layer: 错误的底层(通常是rank)
 * @page_frame_number: 错误发生的页帧号
 * @offset_in_page: 页内偏移
 * @syndrome: 错误校验码(如果适用) 
 * @msg: 错误消息
 * @other_detail: 驱动程序特定的其他错误细节
 */
...
#define mci_for_each_dimm(mci, dimm)       \
    for ((dimm) = (mci)->dimms[0]; (dimm); \
         (dimm) = (dimm)->idx + 1 < (mci)->tot_dimms ? (mci)->dimms[(dimm)->idx + 1] : NULL)
 ...
 */
static inline struct dimm_info *edac_get_dimm(struct mem_ctl_info *mci, int layer0, int layer1, int layer2)
{
    int index;

    if (layer0 < 0 || (mci->n_layers > 1 && layer1 < 0) || (mci->n_layers > 2 && layer2 < 0))
        return NULL;
