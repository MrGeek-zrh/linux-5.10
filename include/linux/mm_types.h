...
struct core_thread { /* coredumqp 线程链表 */
...
struct mmu_gather;
extern void tlb_gather_mmu(struct mmu_gather *tlb, struct mm_struct *mm, unsigned long start, unsigned long end);
extern void tlb_finish_mmu(struct mmu_gather *tlb, unsigned long start, unsigned long end);
...
#define VM_FAULT_ERROR                                                                                 \
    (VM_FAULT_OOM | VM_FAULT_SIGBUS | VM_FAULT_SIGSEGV | VM_FAULT_HWPOISON | VM_FAULT_HWPOISON_LARGE | \
     VM_FAULT_FALLBACK)

#define VM_FAULT_RESULT_TRACE                                                                                         \
    { VM_FAULT_OOM, "OOM" }, { VM_FAULT_SIGBUS, "SIGBUS" }, { VM_FAULT_MAJOR, "MAJOR" }, { VM_FAULT_WRITE, "WRITE" }, \
...
struct vm_special_mapping {
	 ...
	 */
    vm_fault_t (*fault)(const struct vm_special_mapping *sm, struct vm_area_struct *vma, struct vm_fault *vmf);
