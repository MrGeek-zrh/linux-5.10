...
static int submit_bh_wbc(int op, int op_flags, struct buffer_head *bh, enum rw_hint hint,
...
static struct buffer_head *__find_get_block_slow(struct block_device *bdev, sector_t block)
{
    ...
    if (all_mapped && __ratelimit(&last_warned)) {
        printk("__find_get_block_slow() failed. block=%llu, "
               ...
               (unsigned long long)block, (unsigned long long)bh->b_blocknr, bh->b_state, bh->b_size, bdev,
...
{
    struct decrypt_bh_ctx *ctx = container_of(work, struct decrypt_bh_ctx, work);
    struct buffer_head *bh = ctx->bh;
    int err;

    err = fscrypt_decrypt_pagecache_blocks(bh->b_page, bh->b_size, bh_offset(bh));
...
{
    /* Decrypt if needed */
    if (uptodate && fscrypt_inode_uses_fs_layer_crypto(bh->b_page->mapping->host)) {
...
int sync_mapping_buffers(struct address_space *mapping)
{
    ...
    return fsync_buffers_list(&buffer_mapping->private_lock, &mapping->private_list);
}
...
void mark_buffer_dirty_inode(struct buffer_head *bh, struct inode *inode)
{
    ...
    if (!bh->b_assoc_map) {
        ...
        list_move_tail(&bh->b_assoc_buffers, &mapping->private_list);
...
static struct buffer_head *__getblk_slow(struct block_device *bdev, sector_t block, unsigned size, gfp_t gfp)
{
    ...
    if (unlikely(size & (bdev_logical_block_size(bdev) - 1) || (size < 512 || size > PAGE_SIZE))) {
...
void __breadahead_gfp(struct block_device *bdev, sector_t block, unsigned size, gfp_t gfp)
{
...
#define BUFFER_FLAGS_DISCARD (1 << BH_Mapped | 1 << BH_New | 1 << BH_Req | 1 << BH_Delay | 1 << BH_Unwritten)
 ...
 */
int __block_write_full_page(struct inode *inode, struct page *page, get_block_t *get_block,
...
static void iomap_to_bh(struct inode *inode, sector_t block, struct buffer_head *bh, struct iomap *iomap)
{
    ...
    switch (iomap->type) {
		 ...
		 */
            if (!buffer_uptodate(bh) || (offset >= i_size_read(inode)))
                set_buffer_new(bh);
            break;
        case IOMAP_DELALLOC:
            if (!buffer_uptodate(bh) || (offset >= i_size_read(inode)))
        ...
        case IOMAP_MAPPED:
            if ((iomap->flags & IOMAP_F_NEW) || offset >= i_size_read(inode))
                set_buffer_new(bh);
            bh->b_blocknr = (iomap->addr + offset - iomap->offset) >> inode->i_blkbits;
            set_buffer_mapped(bh);
            break;
    }
}
 ...
 */
int block_write_begin(struct address_space *mapping, loff_t pos, unsigned len, unsigned flags, struct page **pagep,
...
int block_write_end(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned copied,
...
int generic_write_end(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned copied,
                      struct page *page, void *fsdata)
{
    struct inode *inode = mapping->host;
...
int block_read_full_page(struct page *page, get_block_t *get_block)
{
    ...
    do {
        ...
        // 当前buffer cache没有建立和磁盘块的映射关系
        // - 这种映射关系主要就是设置b_bdev b_blocknr b_size字段
        if (!buffer_mapped(bh)) {
            int err = 0;
...
int generic_cont_expand_simple(struct inode *inode, loff_t size)
{
    ...
    err = pagecache_write_begin(NULL, mapping, size, 0, AOP_FLAG_CONT_EXPAND, &page, &fsdata);
    if (err)
        goto out;
...
static int cont_expand_zero(struct file *file, struct address_space *mapping, loff_t pos, loff_t *bytes)
{
    ...
    while (index > (curidx = (curpos = *bytes) >> PAGE_SHIFT)) {
        ...
        err = pagecache_write_end(file, mapping, curpos, len, len, page, fsdata);
        if (err < 0)
            goto out;
        BUG_ON(err != len);
        err = 0;
 ...
 */
int cont_write_begin(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned flags,
                     struct page **pagep, void **fsdata, get_block_t *get_block, loff_t *bytes)
{
    struct inode *inode = mapping->host;
    unsigned int blocksize = i_blocksize(inode);
 ...
 * The filesystem needs to handle block truncation upon failure.
 */
int nobh_write_begin(struct address_space *mapping, loff_t pos, unsigned len, unsigned flags, struct page **pagep,
                     void **fsdata, get_block_t *get_block)
{
    ...
    *fsdata = NULL;
	 ...
	 */
    for (block_start = 0, block_in_page = 0, bh = head; block_start < PAGE_SIZE;
        ...
        if (block_start >= to)
            create = 0;
        ret = get_block(inode, block_in_file + block_in_page, bh, create);
        if (ret)
            goto failed;
        if (!buffer_mapped(bh))
            is_mapped_to_disk = 0;
...
int nobh_write_end(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned copied,
                   struct page *page, void *fsdata)
{
    struct inode *inode = page->mapping->host;
 ...
 *
 */
static int submit_bh_wbc(int op, int op_flags, struct buffer_head *bh, enum rw_hint write_hint,
                         struct writeback_control *wbc)
{
    struct bio *bio;
...
{
    return atomic_read(&bh->b_count) | (bh->b_state & ((1 << BH_Dirty) | (1 << BH_Lock)));
}
...
SYSCALL_DEFINE2(bdflush, int, func, long, data)
{
    ...
    if (msg_count < 5) {
        msg_count++;
        printk(KERN_INFO "warning: process `%s' used the obsolete bdflush"
                         " system call\n",
               current->comm);
...
static void recalc_bh_state(void)
{
    ...
    for_each_online_cpu(i) tot += per_cpu(bh_accounting, i).nr;
...
//allocate cache for the `buffer_head`
void __init buffer_init(void)
{
	 ...
	 *
	 * calculate the maximum size of the buffers in memory
	 *  `10%` of the `ZONE_NORMAL`
	 */
    nrpages = (nr_free_buffer_pages() * 10) / 100;
    max_buffer_heads = nrpages * (PAGE_SIZE / sizeof(struct buffer_head));
    ret = cpuhp_setup_state_nocalls(CPUHP_FS_BUFF_DEAD, "fs/buffer:dead", NULL, buffer_exit_cpu_dead);
    WARN_ON(ret < 0);
}
