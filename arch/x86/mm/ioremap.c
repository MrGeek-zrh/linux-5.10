...
int __init arch_ioremap_p4d_supported(void) /* 大页扩展 */
{
	return 0;
}

int __init arch_ioremap_pud_supported(void) /* 大页 */
{
#ifdef CONFIG_X86_64
...
int __init arch_ioremap_pmd_supported(void) /* 大页 */
{
	return boot_cpu_has(X86_FEATURE_PSE/* 页扩展 */);
}
...
//启动时间页表 `bm_pte`
static pte_t __page_aligned_bss bm_pte[PAGE_SIZE/sizeof(pte_t)] ;   //(早期 `ioremap` 页表入口)

//获得 页中间目录条目(`ioremap` 固定映射开始处的页中部目录)
...
void __init early_ioremap_init(void)/* 初始化早期的 `ioremap` */
{
    ...
    //获得了 `FIX_BTMAP_BEGIN` 的页中间目录条目
	pmd = early_ioremap_pmd(fix_to_virt(FIX_BTMAP_BEGIN));

    //把启动时间页表 `bm_pte` 写满 0(早期 `ioremap` 页表入口)
