...
__ref void *alloc_low_pages(unsigned int num)
{
    ...
    if ((pgt_buf_end + num) > pgt_buf_top || !can_use_brk_pgt) {
        ...
        if (min_pfn_mapped < max_pfn_mapped) {
            ret = memblock_find_in_range(min_pfn_mapped << PAGE_SHIFT, max_pfn_mapped << PAGE_SHIFT, PAGE_SIZE * num,
                                         PAGE_SIZE);
        }
        if (ret)
...
static int __meminit save_mr(struct map_range *mr, int nr_range, unsigned long start_pfn, unsigned long end_pfn,
...
static void __ref adjust_range_page_size_mask(struct map_range *mr, int nr_range)
{
    ...
    for (i = 0; i < nr_range; i++) {
        ...
        if ((page_size_mask & (1 << PG_LEVEL_1G)) && !(mr[i].page_size_mask & (1 << PG_LEVEL_1G))) {
...
static const char *page_size_string(struct map_range *mr)
{
	 ...
	 */
    if (IS_ENABLED(CONFIG_X86_32) && !IS_ENABLED(CONFIG_X86_PAE) && mr->page_size_mask & (1 << PG_LEVEL_2M))
        return str_4m;
...
static int __meminit split_mem_range(struct map_range *mr, int nr_range, unsigned long start, unsigned long end)
{
    ...
    if (start_pfn < end_pfn) {
        nr_range =
                save_mr(mr, nr_range, start_pfn, end_pfn, page_size_mask & ((1 << PG_LEVEL_2M) | (1 << PG_LEVEL_1G)));
        pfn = end_pfn;
    }
    ...
    for (i = 0; i < nr_range; i++)
        pr_debug(" [mem %#010lx-%#010lx] page %s\n", mr[i].start, mr[i].end - 1, page_size_string(&mr[i]));
...
static void add_pfn_range_mapped(unsigned long start_pfn, unsigned long end_pfn)
{
    ...
    if (start_pfn < (1UL << (32 - PAGE_SHIFT)))
        max_low_pfn_mapped = max(max_low_pfn_mapped, min(end_pfn, 1UL << (32 - PAGE_SHIFT)));
}
...
void __init poking_init(void)
{
    ...
    if (IS_ENABLED(CONFIG_RANDOMIZE_BASE))
        // 随即化地址
        poking_addr += (kaslr_get_random_long("Poking") & PAGE_MASK) % (TASK_SIZE - TASK_UNMAPPED_BASE - 3 * PAGE_SIZE);
...
__visible DEFINE_PER_CPU_SHARED_ALIGNED(struct tlb_state, cpu_tlbstate) = {
    .loaded_mm = &init_mm, .next_asid = 1, .cr4 = ~0UL, /* fail hard if we screw up cr4 shadow initialization */
};
