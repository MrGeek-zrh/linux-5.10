...
void __init init_vdso_image(const struct vdso_image *image)
{
    ...
    apply_alternatives((struct alt_instr *)(image->data + image->alt),
                       (struct alt_instr *)(image->data + image->alt + image->alt_len));
}
...
static void vdso_fix_landing(const struct vdso_image *image, struct vm_area_struct *new_vma)
{
    ...
    if (in_ia32_syscall() && image == &vdso_image_32) {
        ...
        unsigned long old_land_addr = vdso_land + (unsigned long)current->mm->context.vdso;
...
static int vvar_mremap(const struct vm_special_mapping *sm, struct vm_area_struct *new_vma)
{
...
static vm_fault_t vvar_fault(const struct vm_special_mapping *sm, struct vm_area_struct *vma, struct vm_fault *vmf)
{
	 ...
	 */
        ...
        if (pvti && vclock_was_used(VDSO_CLOCKMODE_PVCLOCK)) {
            return vmf_insert_pfn_prot(vma, vmf->address, __pa(pvti) >> PAGE_SHIFT,
	 ...
	 */
        ...
        if (tsc_pg && vclock_was_used(VDSO_CLOCKMODE_HVCLOCK))
            return vmf_insert_pfn(vma, vmf->address, virt_to_phys(tsc_pg) >> PAGE_SHIFT);
        /**
	 * timens
	 */
...
static int map_vdso(const struct vdso_image *image, unsigned long addr)
{
    ...
    addr = get_unmapped_area(NULL, addr, image->size - image->sym_vvar_start, 0, 0);
    ...
    vma = _install_special_mapping(mm, text_start, image->size,
                                   VM_READ | VM_EXEC | VM_MAYREAD | VM_MAYWRITE | VM_MAYEXEC, &vdso_mapping);
    ...
    vma = _install_special_mapping(mm, addr, -image->sym_vvar_start,
                                   VM_READ | VM_MAYREAD | VM_IO | VM_DONTDUMP | VM_PFNMAP, &vvar_mapping);
...
int map_vdso_once(const struct vdso_image *image, unsigned long addr)
{
	 ...
	 */
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
        if (vma_is_special_mapping(vma, &vdso_mapping) || vma_is_special_mapping(vma, &vvar_mapping)) {
            mmap_write_unlock(mm);
            return -EEXIST;
        }
    }
