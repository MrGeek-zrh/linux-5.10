...
static void __init reserve_initrd(void)
{
    ...
    /**
	 * 这实际上显示的是 /boot/initramfs-xxx.img 文件的大小
	 *
	 * 我的环境：
	 *
	 * RAMDISK: [mem 0x5f66a000-0x63a22fff] ~= 67.72265529632568M
	 *
	 * $ sudo du -sh /boot/initramfs-5.14.0-229.el9.x86_64.img
	 * 68M	/boot/initramfs-5.14.0-229.el9.x86_64.img
	 */
...
static void __init memblock_x86_reserve_range_setup_data(void)
{
    ...
    while (pa_data) {
        ...
        if (data->type == SETUP_INDIRECT && ((struct setup_indirect *)data->data)->type != SETUP_INDIRECT)
            memblock_reserve(((struct setup_indirect *)data->data)->addr, ((struct setup_indirect *)data->data)->len);
...
static int __init reserve_crashkernel_low(void)
{
    ...
    pr_info("Reserving %ldMB of low memory at %ldMB for crashkernel (low RAM limit: %ldMB)\n",
            (unsigned long)(low_size >> 20), (unsigned long)(low_base >> 20), (unsigned long)(low_mem_limit >> 20));
...
static void __init reserve_crashkernel(void)
{
    ...
    if (!crash_base) {
		 ...
		 */
        if (!high)
            crash_base = memblock_phys_alloc_range(crash_size, CRASH_ALIGN, CRASH_ALIGN, CRASH_ADDR_LOW_MAX);
        if (!crash_base)
            crash_base = memblock_phys_alloc_range(crash_size, CRASH_ALIGN, CRASH_ALIGN, CRASH_ADDR_HIGH_MAX);
        if (!crash_base) {
        ...
        start = memblock_phys_alloc_range(crash_size, SZ_1M, crash_base, crash_base + crash_size);
        if (start != crash_base) {
    ...
    pr_info("Reserving %ldMB of memory at %ldMB for crashkernel (System RAM: %ldMB)\n",
            (unsigned long)(crash_size >> 20), (unsigned long)(crash_base >> 20), (unsigned long)(total_mem >> 20));
...
// 初始化efi
    ...
    //
    // boot_cpu_data.x86_phys_bits 表示 CPU 支持的物理地址位数。
    // 1ULL << boot_cpu_data.x86_phys_bits 计算出了 CPU 可寻址的最大物理地址
