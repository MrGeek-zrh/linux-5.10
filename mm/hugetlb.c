...
int __read_mostly hugetlb_max_hstate;
...
struct mutex ____cacheline_aligned_in_smp *hugetlb_fault_mutex_table;
...
static void copy_hugetlb_cgroup_uncharge_info(struct file_region *nrg, struct file_region *rg)
{
...
static void record_hugetlb_cgroup_uncharge_info(struct hugetlb_cgroup *h_cg, struct hstate *h, struct resv_map *resv,
                                                struct file_region *nrg)
{
#ifdef CONFIG_CGROUP_HUGETLB
    if (h_cg) {
        nrg->reservation_counter = &h_cg->rsvd_hugepage[hstate_index(h)];
        nrg->css = &h_cg->css;
...
static bool has_same_uncharge_info(struct file_region *rg, struct file_region *org)
{
#ifdef CONFIG_CGROUP_HUGETLB
    return rg && org && rg->reservation_counter == org->reservation_counter && rg->css == org->css;
...
static void coalesce_file_region(struct resv_map *resv, struct file_region *rg)
{
    ...
    if (&prg->link != &resv->regions && prg->to == rg->from && has_same_uncharge_info(prg, rg)) {
        prg->to = rg->to;
    ...
    if (&nrg->link != &resv->regions && nrg->from == rg->to && has_same_uncharge_info(nrg, rg)) {
        nrg->from = rg->from;
 ...
 */
static long add_reservation_in_range(struct resv_map *resv, long f, long t, struct hugetlb_cgroup *h_cg,
                                     struct hstate *h, long *regions_needed)
{
    ...
    list_for_each_entry_safe(rg, trg, head, link)
    {
        ...
        if (rg->from > last_accounted_offset) {
            ...
            if (!regions_needed) {
                nrg = get_file_region_entry_from_cache(resv, last_accounted_offset, rg->from);
    ...
    if (last_accounted_offset < t) {
        ...
        if (!regions_needed) {
            nrg = get_file_region_entry_from_cache(resv, last_accounted_offset, t);
            record_hugetlb_cgroup_uncharge_info(h_cg, h, resv, nrg);
            list_add(&nrg->link, rg->link.prev);
            coalesce_file_region(resv, nrg);
        } else
 ...
 */
static int allocate_file_region_entries(struct resv_map *resv, int regions_needed) __must_hold(&resv->lock)
{
	 ...
	 */
    while (resv->region_cache_count < (resv->adds_in_progress + regions_needed)) {
        to_allocate = resv->adds_in_progress + regions_needed - resv->region_cache_count;
 ...
 */
static long region_add(struct resv_map *resv, long f, long t, long in_regions_needed, struct hstate *h,
...
retry:
    ...
    if (actual_regions_needed > in_regions_needed &&
        resv->region_cache_count < resv->adds_in_progress + (actual_regions_needed - in_regions_needed)) {
        /* region_add operation of range 1 should never need to
		 * allocate file_region entries.
		 */
        VM_BUG_ON(t - f <= 1);

        if (allocate_file_region_entries(resv, actual_regions_needed - in_regions_needed)) {
            return -ENOMEM;
        }
 ...
 */
static long region_chg(struct resv_map *resv, long f, long t, long *out_regions_needed)
{
    long chg = 0;
 ...
 */
static void region_abort(struct resv_map *resv, long f, long t, long regions_needed)
{
    spin_lock(&resv->lock);
...
retry:
    ...
    list_for_each_entry_safe(rg, trg, head, link)
    {
        ...
        if (f > rg->from && t < rg->to) { /* Must split region */
			 ...
			 */
            if (!nrg && resv->region_cache_count > resv->adds_in_progress) {
                nrg = list_first_entry(&resv->region_cache, struct file_region, link);
                list_del(&nrg->link);
                resv->region_cache_count--;
            }
            ...
            del += t - f;
            hugetlb_cgroup_uncharge_file_region(resv, rg, t - f);
        ...
        if (f <= rg->from && t >= rg->to) { /* Remove entire region */
            del += rg->to - rg->from;
            hugetlb_cgroup_uncharge_file_region(resv, rg, rg->to - rg->from);
            list_del(&rg->link);
            kfree(rg);
            continue;
        }
        ...
        if (f <= rg->from) { /* Trim beginning of region */
            hugetlb_cgroup_uncharge_file_region(resv, rg, t - rg->from);

            del += t - rg->from;
            rg->from = t;
        } else { /* Trim end of region */
            hugetlb_cgroup_uncharge_file_region(resv, rg, rg->to - f);
...
static void resv_map_set_hugetlb_cgroup_uncharge_info(struct resv_map *resv_map, struct hugetlb_cgroup *h_cg,
                                                      struct hstate *h)
{
#ifdef CONFIG_CGROUP_HUGETLB
    if (!h_cg || !h) {
        ...
        resv_map->reservation_counter = &h_cg->rsvd_hugepage[hstate_index(h)];
...
static struct page *dequeue_huge_page_vma(struct hstate *h, struct vm_area_struct *vma, unsigned long address,
                                          int avoid_reserve, long chg)
{
    struct page *page;
	 ...
	 */
    if (!vma_has_reserves(vma, chg) && h->free_huge_pages - h->resv_huge_pages == 0)
        goto err;
...
#define for_each_node_mask_to_alloc(hs, nr_nodes, node, mask)                                                 \
    for (nr_nodes = nodes_weight(*mask); nr_nodes > 0 && ((node = hstate_next_node_to_alloc(hs, mask)) || 1); \
         nr_nodes--)

#define for_each_node_mask_to_free(hs, nr_nodes, node, mask) \
    for (nr_nodes = nodes_weight(*mask); nr_nodes > 0 && ((node = hstate_next_node_to_free(hs, mask)) || 1); nr_nodes--)
...
static struct page *alloc_gigantic_page(struct hstate *h, gfp_t gfp_mask, int nid, nodemask_t *nodemask)
{
...
#ifdef CONFIG_CMA
    {
        ...
        if (hugetlb_cma[nid]) {
            page = cma_alloc(hugetlb_cma[nid], nr_pages, huge_page_order(h), true);
            if (page)
                return page;
        }
        ...
        if (!(gfp_mask & __GFP_THISNODE)) {
            for_each_node_mask(node, *nodemask)
            {
                ...
                page = cma_alloc(hugetlb_cma[node], nr_pages, huge_page_order(h), true);
                if (page)
                    return page;
            }
        }
...
static void update_and_free_page(struct hstate *h, struct page *page)
{
    ...
    for (i = 0; i < pages_per_huge_page(h); i++) {
        page[i].flags &= ~(1 << PG_locked | 1 << PG_error | 1 << PG_referenced | 1 << PG_dirty | 1 << PG_active |
...
static void __free_huge_page(struct page *page)
{
    ...
    hugetlb_cgroup_uncharge_page_rsvd(hstate_index(h), pages_per_huge_page(h), page);
    if (restore_reserve)
        h->resv_huge_pages++;
...
static void free_hpage_workfn(struct work_struct *work)
{
    ...
    while (node) {
        page = container_of((struct address_space **)node, struct page, mapping);
        node = node->next;
                                          ...
                                          nodemask_t *node_alloc_noretry)
{
    ...
    if (hstate_is_gigantic(h))
        ...
        page = alloc_buddy_huge_page(h, gfp_mask, nid, nmask, node_alloc_noretry);
    if (!page)
        return NULL;
...
static int free_pool_huge_page(struct hstate *h, nodemask_t *nodes_allowed, bool acct_surplus)
{
    ...
    for_each_node_mask_to_free(h, nr_nodes, node, nodes_allowed)
    {
        ...
        if ((!acct_surplus || h->surplus_huge_pages_node[node]) && !list_empty(&h->hugepage_freelists[node])) {
            struct page *page = list_entry(h->hugepage_freelists[node].next, struct page, lru);
            list_del(&page->lru);
            h->free_huge_pages--;
 ...
 * of size 'delta'.
 */
static int gather_surplus_pages(struct hstate *h, int delta) __must_hold(&hugetlb_lock)
{
...
retry:
    spin_unlock(&hugetlb_lock);
    for (i = 0; i < needed; i++) {
        page = alloc_surplus_huge_page(h, htlb_alloc_mask(h), NUMA_NO_NODE, NULL);
        if (!page) {
            alloc_ok = false;
            break;
        }
 ...
 */
static void restore_reserve_on_error(struct hstate *h, struct vm_area_struct *vma, unsigned long address,
                                     struct page *page)
{
...
struct page *alloc_huge_page(struct vm_area_struct *vma, unsigned long addr, int avoid_reserve)
{
	 ...
	 */
    if (deferred_reserve) {
        hugetlb_cgroup_commit_charge_rsvd(idx, pages_per_huge_page(h), h_cg, page);
    }
...
int alloc_bootmem_huge_page(struct hstate *h) __attribute__((weak, alias("__alloc_bootmem_huge_page")));
int __alloc_bootmem_huge_page(struct hstate *h)
{
    ...
    for_each_node_mask_to_alloc(h, nr_nodes, node, &node_states[N_MEMORY])
    {
        ...
        addr = memblock_alloc_try_nid_raw(huge_page_size(h), huge_page_size(h), 0, MEMBLOCK_ALLOC_ACCESSIBLE, node);
        if (addr) {
            /*
...
static void __init hugetlb_hstate_alloc_pages(struct hstate *h)
{
    ...
    if (!hstate_is_gigantic(h)) {
		 ...
		 */
        node_alloc_noretry = kmalloc(sizeof(*node_alloc_noretry), GFP_KERNEL);
    } else {
    ...
    for (i = 0; i < h->max_huge_pages; ++i) {
        if (hstate_is_gigantic(h)) {
            ...
            if (!alloc_bootmem_huge_page(h))
                break;
        } else if (!alloc_pool_huge_page(h, &node_states[N_MEMORY], node_alloc_noretry))
            break;
        cond_resched();
    }
...
static int adjust_pool_surplus(struct hstate *h, nodemask_t *nodes_allowed, int delta)
{
    ...
    if (delta < 0) {
        ...
        for_each_node_mask_to_free(h, nr_nodes, node, nodes_allowed)
        {
            if (h->surplus_huge_pages_node[node] < h->nr_huge_pages_node[node])
                goto found;
        }
    }
    return 0;
...
static int set_max_huge_pages(struct hstate *h, unsigned long count, int nid, nodemask_t *nodes_allowed)
{
...
#define HSTATE_ATTR(_name) static struct kobj_attribute _name##_attr = __ATTR(_name, 0644, _name##_show, _name##_store)
...
static ssize_t __nr_hugepages_store_common(bool obey_mempolicy, struct hstate *h, int nid, unsigned long count,
                                           size_t len)
{
    int err;
...
static ssize_t nr_hugepages_mempolicy_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                            size_t len)
{
...
static ssize_t nr_overcommit_hugepages_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                             size_t count)
{
    int err;
    unsigned long input;
...
static void hugetlb_register_node(struct node *node)
{
    ...
    nhs->hugepages_kobj = kobject_create_and_add("hugepages", &node->dev.kobj);
    if (!nhs->hugepages_kobj)
        return;

    for_each_hstate(h)
    {
        err = hugetlb_sysfs_add_hstate(h, nhs->hugepages_kobj, nhs->hstate_kobjs, &per_node_hstate_attr_group);
        if (err) {
...
static void __init hugetlb_register_all_nodes(void)
{
	 ...
	 */
    register_hugetlbfs_with_node(hugetlb_register_node, hugetlb_unregister_node);
}
#else /* !CONFIG_NUMA */
...
static int __init hugepagesz_setup(char *s)
{
    ...
    if (h) {
		 ...
		 */
        if (!parsed_default_hugepagesz || h != &default_hstate || default_hstate.max_huge_pages) {
...
static unsigned int allowed_mems_nr(struct hstate *h)
{
    ...
    {
        if (!mpol_allowed || (mpol_allowed && node_isset(node, *mpol_allowed)))
            nr += array[node];
    }
...
#ifdef CONFIG_SYSCTL
static int proc_hugetlb_doulongvec_minmax(struct ctl_table *table, int write, void *buffer, size_t *length,
...
static int hugetlb_sysctl_handler_common(bool obey_mempolicy, struct ctl_table *table, int write, void *buffer,
...
#ifdef CONFIG_NUMA
int hugetlb_mempolicy_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length, loff_t *ppos)
{
    return hugetlb_sysctl_handler_common(true, table, write, buffer, length, ppos);
}
#endif /* CONFIG_NUMA */
    ...
    for_each_hstate(h)
    {
        ...
        if (h == &default_hstate)
            seq_printf(m,
                       ...
                       "HugePages_Surp:    %5lu\n"
                       "Hugepagesize:   %8lu kB\n",
                       count, h->free_huge_pages, h->resv_huge_pages, h->surplus_huge_pages,
...
int hugetlb_report_node_meminfo(char *buf, int len, int nid)
{
    ...
    return sysfs_emit_at(buf, len,
                         ...
                         nid, h->nr_huge_pages_node[nid], nid, h->free_huge_pages_node[nid], nid,
                         h->surplus_huge_pages_node[nid]);
}
...
void hugetlb_show_meminfo(void)
{
    ...
    for_each_node_state(nid, N_MEMORY) for_each_hstate(h)
            pr_info("Node %d hugepages_total=%u hugepages_free=%u hugepages_surp=%u hugepages_size=%lukB\n", nid,
                    h->nr_huge_pages_node[nid], h->free_huge_pages_node[nid], h->surplus_huge_pages_node[nid],
...
unsigned long hugetlb_total_pages(void)
{
    ...
    for_each_hstate(h) nr_total_pages += h->nr_huge_pages * pages_per_huge_page(h);
...
int copy_hugetlb_page_range(struct mm_struct *dst, struct mm_struct *src, struct vm_area_struct *vma)
{
    ...
    if (cow) {
        mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, src, vma->vm_start, vma->vm_end);
    ...
    for (addr = vma->vm_start; addr < vma->vm_end; addr += sz) {
        ...
        if (huge_pte_none(entry) || !huge_pte_none(dst_entry)) {
			 ...
			 */
            ;
        } else if (unlikely(is_hugetlb_entry_migration(entry) || is_hugetlb_entry_hwpoisoned(entry))) {
 ...
 * same region.
 */
static void unmap_ref_private(struct mm_struct *mm, struct vm_area_struct *vma, struct page *page,
                              unsigned long address)
{
    struct hstate *h = hstate_vma(vma);
 ...
 */
static vm_fault_t hugetlb_cow(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long address, pte_t *ptep,
...
retry_avoidcopy:
	 ...
	 */
    if (is_vma_resv_set(vma, HPAGE_RESV_OWNER) && old_page != pagecache_page)
        outside_reserve = 1;
    ...
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, mm, haddr, haddr + huge_page_size(h));
    mmu_notifier_invalidate_range_start(&range);
    ...
    if (likely(ptep && pte_same(huge_ptep_get(ptep), pte))) {
        ...
        set_huge_pte_at(mm, haddr, ptep, make_huge_pte(vma, new_page, 1));
...
static vm_fault_t hugetlb_no_page(struct mm_struct *mm, struct vm_area_struct *vma, struct address_space *mapping,
                                  pgoff_t idx, unsigned long address, pte_t *ptep, unsigned int flags)
{
    struct hstate *h = hstate_vma(vma);
...
retry:
    ...
    if (!page) {
        ...
        if (userfaultfd_missing(vma)) {
            u32 hash;
            struct vm_fault vmf = {
                .vma = vma, .address = haddr, .flags = flags,
                /*
...
vm_fault_t hugetlb_fault(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long address, unsigned int flags)
{
    ...
    struct hstate *h = hstate_vma(vma); /* vma->file->inode->superblock->privateinfo->hstate */
    struct address_space *mapping;
    int need_wait_lock = 0;
    unsigned long haddr = address & huge_page_mask(h); /* hugepage地址 */
    ...
    if (ptep) {
        ...
        if (unlikely(is_hugetlb_entry_migration(entry))) {
            ...
            return VM_FAULT_HWPOISON_LARGE | VM_FAULT_SET_HINDEX(hstate_index(h));
    }
    ...
    entry = pte_mkyoung(entry);
    if (huge_ptep_set_access_flags(vma, haddr, ptep, entry, flags & FAULT_FLAG_WRITE))
                             ...
                             unsigned long dst_addr, unsigned long src_addr, struct page **pagep)
{
    ...
    if (!*pagep) {
        ...
        ret = copy_huge_page_from_user(page, (const void __user *)src_addr, pages_per_huge_page(h), false);
    ...
    (void)huge_ptep_set_access_flags(dst_vma, dst_addr, dst_pte, _dst_pte, dst_vma->vm_flags & VM_WRITE);
...
long follow_hugetlb_page(struct mm_struct *mm, struct vm_area_struct *vma, struct page **pages,
                         struct vm_area_struct **vmas, unsigned long *position, unsigned long *nr_pages, long i,
                         unsigned int flags, int *locked)
{
    ...
    while (vaddr < vma->vm_end && remainder) {
		 ...
		 */
        if (!pages && !vmas && !pfn_offset && (vaddr + huge_page_size(h) < vma->vm_end) &&
    ...
    same_page:
        ...
        --remainder;
        ++i;
        if (vaddr < vma->vm_end && remainder && pfn_offset < pages_per_huge_page(h)) {
            /*
...
int hugetlb_reserve_pages(struct inode *inode, long from, long to, struct vm_area_struct *vma, vm_flags_t vm_flags)
{
    ...
    ret = hugetlb_cgroup_charge_cgroup_rsvd(hstate_index(h), chg * pages_per_huge_page(h), &h_cg);
    ...
    if (!vma || vma->vm_flags & VM_MAYSHARE) {
        ...
        if (unlikely(add < 0)) {
            ...
            hugetlb_cgroup_uncharge_cgroup_rsvd(hstate_index(h), (chg - add) * pages_per_huge_page(h), h_cg);
...
static unsigned long page_table_shareable(struct vm_area_struct *svma, struct vm_area_struct *vma, unsigned long addr,
                                          pgoff_t idx)
{
...
struct page *__weak follow_huge_pd(struct vm_area_struct *vma, unsigned long address, hugepd_t hpd, int flags,
                                   int pdshift)
{
...
void __init hugetlb_cma_reserve(int order)
{
    ...
    for_each_node_state(nid, N_ONLINE)
    {
        ...
        res = cma_declare_contiguous_nid(0, size, 0, PAGE_SIZE << order, 0, false, name, &hugetlb_cma[nid], nid);
        if (res) {
            pr_warn("hugetlb_cma: reservation failed: err %d, node %d", res, nid);
            continue;
        }
