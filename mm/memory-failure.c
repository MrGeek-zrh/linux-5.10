...
int __read_mostly sysctl_memory_failure_early_kill = 0;

int __read_mostly sysctl_memory_failure_recovery = 1;
...
static int hwpoison_filter_dev(struct page *p)
{
    ...
    if (hwpoison_filter_dev_major == ~0U && hwpoison_filter_dev_minor == ~0U)
        return 0;
    ...
    if (hwpoison_filter_dev_major != ~0U && hwpoison_filter_dev_major != MAJOR(dev))
        return -EINVAL;
    if (hwpoison_filter_dev_minor != ~0U && hwpoison_filter_dev_minor != MINOR(dev))
        return -EINVAL;
...
static int kill_proc(struct to_kill *tk, unsigned long pfn, int flags)
{
    ...
    if (flags & MF_ACTION_REQUIRED) {
        ...
        ret = force_sig_mceerr(BUS_MCEERR_AR, (void __user *)tk->addr, addr_lsb);
    } else {
        /*
		 ...
		 */
        ret = send_sig_mceerr(BUS_MCEERR_AO, (void __user *)tk->addr, addr_lsb, t); /* synchronous? */
    }
    if (ret < 0)
...
static unsigned long dev_pagemap_mapping_shift(struct page *page, struct vm_area_struct *vma)
{
 ...
 */
static void add_to_kill(struct task_struct *tsk, struct page *p, struct vm_area_struct *vma, struct list_head *to_kill)
{
    struct to_kill *tk;
 ...
 */
static void collect_procs_anon(struct page *page, struct list_head *to_kill, int force_early)
{
 ...
 */
static void collect_procs_file(struct page *page, struct list_head *to_kill, int force_early)
{
    struct vm_area_struct *vma;
    struct task_struct *tsk;
...
static int truncate_error_page(struct page *p, unsigned long pfn, struct address_space *mapping)
{
    int ret = MF_FAILED;
...
static int page_action(struct page_state *ps, struct page *p, unsigned long pfn)
{
    int result;
    int count;
 ...
 */
static bool hwpoison_user_mappings(struct page *p, unsigned long pfn, int flags, struct page **hpagep)
{
	 ...
	 */
    mapping = page_mapping(hpage);
    if (!(flags & MF_MUST_KILL) && !PageDirty(hpage) && mapping && mapping_can_writeback(mapping)) {
...
static int memory_failure_hugetlb(unsigned long pfn, int flags)
{
    ...
    if (!(flags & MF_COUNT_INCREASED) && !get_hwpoison_page(p)) {
        ...
        if (PageHWPoison(head)) {
            if ((hwpoison_filter(p) && TestClearPageHWPoison(p)) || (p != head && TestSetPageHWPoison(head))) {
...
static int memory_failure_dev_pagemap(unsigned long pfn, int flags, struct dev_pagemap *pgmap)
{
    ...
    list_for_each_entry(tk, &tokill, nd) if (tk->size_shift) size = max(size, 1UL << tk->size_shift);
    if (size) {
        /*
