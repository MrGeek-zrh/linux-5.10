...
unsigned long zone_reclaimable_pages(struct zone *zone)
{
     ...
     *  1. 不活跃 + 活跃 的文件映射
     */
    nr = zone_page_state_snapshot(zone, NR_ZONE_INACTIVE_FILE) + zone_page_state_snapshot(zone, NR_ZONE_ACTIVE_FILE);
    /**
     *  2. 如果开启了 swap
     */
...
static unsigned long do_shrink_slab(struct shrink_control *shrinkctl, struct shrinker *shrinker, int priority)
{
    ...
    trace_mm_shrink_slab_start(shrinker, shrinkctl, nr, freeable, delta, total_scan, priority);
	 ...
	 */
    if (next_deferred > 0)
        new_nr = atomic_long_add_return(next_deferred, &shrinker->nr_deferred[nid]);
    else
...
static unsigned long shrink_slab_memcg(gfp_t gfp_mask, int nid, struct mem_cgroup *memcg, int priority)
{
    ...
    for_each_set_bit(i, map->map, shrinker_nr_max)
    {
        ...
        if (!memcg_kmem_enabled() && !(shrinker->flags & SHRINKER_NONSLAB))
            continue;
...
static pageout_t pageout(struct page *page, struct address_space *mapping)
{
    ...
    if (clear_page_dirty_for_io(page)) {
        ...
        struct writeback_control wbc = {
            wbc.sync_mode = WB_SYNC_NONE, wbc.nr_to_write = SWAP_CLUSTER_MAX,
                            ...
                            struct mem_cgroup *target_memcg)
{
     ...
     */
		 ...
		 * same address_space.
		 */
        if (reclaimed && page_is_file_lru(page) && !mapping_exiting(mapping) && !dax_mapping(mapping))
        ...
        if (PageAnon(page) && PageSwapBacked(page)) {
            ...
            if (!PageSwapCache(page)) {
                ...
                if (PageTransHuge(page)) {
					 ...
					 */
                    if (!compound_mapcount(page) && split_huge_page_to_list(page, page_list))
                        goto activate_locked;
                }
                /**
                 *  分配交换空间
...
unsigned int reclaim_clean_pages_from_list(struct zone *zone, struct list_head *page_list)
{
    ...
    {
        if (page_is_file_lru(page) && !PageDirty(page) && !__PageMovable(page) && !PageUnevictable(page)) {
            ClearPageActive(page);
            list_move(&page->lru, &clean_pages);
        }
    }

    nr_reclaimed = shrink_page_list(&clean_pages, zone->zone_pgdat, &sc, &stat, true);
                                       ...
                                       unsigned long *nr_scanned, struct scan_control *sc, enum lru_list lru)
{
    ...
    *nr_scanned = total_scan;
    trace_mm_vmscan_lru_isolate(sc->reclaim_idx, sc->order, nr_to_scan, total_scan, skipped, nr_taken, mode, lru);
...
{
    return !(current->flags & PF_LOCAL_THROTTLE) || current->backing_dev_info == NULL ||
 ...
 * 扫描不活跃链表 并且 尝试 回收页面
 */
static noinline_for_stack unsigned long shrink_inactive_list(unsigned long nr_to_scan, struct lruvec *lruvec,
                                                             struct scan_control *sc, enum lru_list lru)
{
    ...
    trace_mm_vmscan_lru_shrink_inactive(pgdat->node_id, nr_scanned, nr_reclaimed, &stat, sc->priority, file);
    return nr_reclaimed;
}
                               ...
                               enum lru_list lru)
{
	 ...
	 *
	 */
    trace_mm_vmscan_lru_shrink_active(pgdat->node_id, nr_taken, nr_activate, nr_deactivate, nr_rotated, sc->priority,
                                      file);
}
...
unsigned long reclaim_pages(struct list_head *page_list)
{
    ...
    while (!list_empty(page_list)) {
        ...
        nr_reclaimed += shrink_page_list(&node_page_list, NODE_DATA(nid), &sc, &dummy_stat, false);
 ...
 */
static unsigned long shrink_list(enum lru_list lru, unsigned long nr_to_scan, struct lruvec *lruvec,
                                 struct scan_control *sc)
{
    /**
     *  活跃的 匿名 或 文件映射
...
out:
    ...
    for_each_evictable_lru(lru)
    {
        ...
        protection = mem_cgroup_protection(sc->target_mem_cgroup, memcg, sc->memcg_low_reclaim);
        ...
        if (protection) {
            ...
            scan = lruvec_size - lruvec_size * protection / cgroup_size;
        ...
        switch (scan_balance) {
			 ...
			 * round-off error.
			 */
                scan = mem_cgroup_online(memcg) ? div64_u64(scan * fraction[file], denominator) :
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	 ...
	 */
    scan_adjusted = (!cgroup_reclaim(sc) && !current_is_kswapd() && sc->priority == DEF_PRIORITY);
 ...
 */
static inline bool should_continue_reclaim(struct pglist_data *pgdat, unsigned long nr_reclaimed,
                                           struct scan_control *sc)
{
...
again:
    ...
    if (!sc->force_deactivate) {
        ...
        if (refaults != target_lruvec->refaults[0] || inactive_is_low(target_lruvec, LRU_INACTIVE_ANON))
        ...
        if (refaults != target_lruvec->refaults[1] || inactive_is_low(target_lruvec, LRU_INACTIVE_FILE))
    ...
    if (!cgroup_reclaim(sc)) {
         ...
         * 2.
         */
        sc->file_is_tiny = file + free <= total_high_wmark && !(sc->may_deactivate & DEACTIVATE_ANON) &&
                           anon >> sc->priority;
    }
	 ...
	 *  判断内存压力
	 */
    vmpressure(sc->gfp_mask, sc->target_mem_cgroup, true, sc->nr_scanned - nr_scanned, sc->nr_reclaimed - nr_reclaimed);
	 ...
	 */
    if ((current_is_kswapd() || (cgroup_reclaim(sc) && writeback_throttling_sane(sc))) && sc->nr.dirty &&
        sc->nr.dirty == sc->nr.congested)
	 ...
	 */
    if (!current_is_kswapd() && current_may_throttle() && !sc->hibernation_mode &&
...
static void shrink_zones(struct zonelist *zonelist, struct scan_control *sc)
{
    ...
    for_each_zone_zonelist_nodemask(zone, z, zonelist, sc->reclaim_idx, sc->nodemask)
    {
        ...
        if (!cgroup_reclaim(sc)) {
            ...
            nr_soft_scanned = 0;
            nr_soft_reclaimed =
                    mem_cgroup_soft_limit_reclaim(zone->zone_pgdat, sc->order, sc->gfp_mask, &nr_soft_scanned);
...
static bool throttle_direct_reclaim(gfp_t gfp_mask, struct zonelist *zonelist, nodemask_t *nodemask)
{
    ...
    wait_event_killable(zone->zone_pgdat->pfmemalloc_wait, allow_direct_reclaim(pgdat));
...
unsigned long mem_cgroup_shrink_node(struct mem_cgroup *memcg, gfp_t gfp_mask, bool noswap, pg_data_t *pgdat,
                                     unsigned long *nr_scanned)
{
    struct lruvec *lruvec = mem_cgroup_lruvec(memcg, pgdat);
    struct scan_control sc = {
 ...
 *
 */
unsigned long try_to_free_mem_cgroup_pages(struct mem_cgroup *memcg, unsigned long nr_pages, gfp_t gfp_mask,
                                           bool may_swap)
{
    unsigned long nr_reclaimed;
    ...
    struct scan_control sc = {
        ...
        .gfp_mask = (current_gfp_context(gfp_mask) & GFP_RECLAIM_MASK) | (GFP_HIGHUSER_MOVABLE & ~GFP_RECLAIM_MASK),
        .reclaim_idx = MAX_NR_ZONES - 1,
        .target_mem_cgroup = memcg,
        .priority = DEF_PRIORITY,
        .may_writepage = !laptop_mode,
