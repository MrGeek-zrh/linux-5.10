...
void __ClearPageMovable(struct page *page)
{
	 ...
	 */
    page->mapping = (void *)((unsigned long)page->mapping & PAGE_MAPPING_MOVABLE);
}
...
static inline bool isolation_suitable(struct compact_control *cc, struct page *page)
{
...
static bool __reset_isolation_pfn(struct zone *zone, unsigned long pfn, bool check_source, bool check_target)
{
	 ...
	 */
    if (!check_source && check_target && get_pageblock_migratetype(page) != MIGRATE_MOVABLE)
        return false;
...
static void __reset_isolation_suitable(struct zone *zone)
{
	 ...
	 */
    for (; migrate_pfn < free_pfn; migrate_pfn += pageblock_nr_pages, free_pfn -= pageblock_nr_pages) {
        cond_resched();
 ...
 */
static bool test_and_set_skip(struct compact_control *cc, struct page *page, unsigned long pfn)
{
    bool skip;
 ...
 */
static void update_pageblock_skip(struct compact_control *cc, struct page *page, unsigned long pfn)
{
    struct zone *zone = cc->zone;
 ...
 */
static bool compact_lock_irqsave(spinlock_t *lock, unsigned long *flags, struct compact_control *cc) __acquires(lock)
{
    /**
     *  异步模式，如果有人已经获取了锁，那么只能退出
     */
 ...
 */
static unsigned long isolate_freepages_block(struct compact_control *cc, unsigned long *start_pfn,
                                             unsigned long end_pfn, struct list_head *freelist, unsigned int stride,
                                             bool strict)
{
    ...
    for (; blockpfn < end_pfn; blockpfn += stride, cursor += stride) {
		 ...
		 */
        if (!(blockpfn % SWAP_CLUSTER_MAX) && compact_unlock_should_abort(&cc->zone->lock, flags, &locked, cc))
            break;
...
unsigned long isolate_freepages_range(struct compact_control *cc, unsigned long start_pfn, unsigned long end_pfn)
{
    ...
    for (; pfn < end_pfn; pfn += isolated, block_start_pfn = block_end_pfn, block_end_pfn += pageblock_nr_pages) {
        ...
        isolated = isolate_freepages_block(cc, &isolate_start_pfn, block_end_pfn, &freelist, 0, true);
...
static bool too_many_isolated(pg_data_t *pgdat)
{
     ...
     *  不活跃
     */
    inactive = node_page_state(pgdat, NR_INACTIVE_FILE) + node_page_state(pgdat, NR_INACTIVE_ANON);

    /**
     *  活跃
     */
    active = node_page_state(pgdat, NR_ACTIVE_FILE) + node_page_state(pgdat, NR_ACTIVE_ANON);

    /**
     *  隔离
     */
    isolated = node_page_state(pgdat, NR_ISOLATED_FILE) + node_page_state(pgdat, NR_ISOLATED_ANON);
 ...
 *  对页块 里的 页面执行分离任务
 */
static unsigned long isolate_migratepages_block(struct compact_control *cc, unsigned long low_pfn,
                                                unsigned long end_pfn, isolate_mode_t isolate_mode)
{
    ...
    /**
     *  如果这次内存规整是`直接内存规整`，并且是异步模式，这种情况，可以做一点优化
     *
     *   以其实页帧号 low_pfn 为起点，以 2^cc->order 个连续页面为终点，
     *      在此范围内，若发现不合适做内存规整的页面，则跳出这个范围。
     *  所以，把 skip_on_failure = true
     */
    ...
    for (; low_pfn < end_pfn; low_pfn++) {
		 ...
		 */
        if (!(low_pfn % SWAP_CLUSTER_MAX) && compact_unlock_should_abort(&pgdat->lru_lock, flags, &locked, cc)) {
            low_pfn = 0;
            goto fatal_pending;
        }
		 ...
		 *
		 * 混合页面也不适合做内存规整，如透明 大页 以及 hugetlbfs
		 */
		 ...
		 *
		 * 处理不在 LRU 链表 中的页面的情况，不在 LRU 链表不适合做迁移。
		 *  但是有个特殊情况，见下
		 */
        if (!PageLRU(page)) {
            /*
			 ...
			 *
			 * 非 LRU 页面，并且 还 没有分离的页面，是可以迁移的。
			 *  这些页面通常是特殊的可迁移页面，如 zsmalloc()
			 */
		 ...
		 *
		 * 允许在 GFP_NOFS 情况下迁移匿名页面，因为他不会 依赖文件系统的锁。
		 */
        ...
        if (!locked) {
            /**
             *  尝试获取 zone->zone_pgdat->lru_lock 的锁
             */
            locked = compact_lock_irqsave(&pgdat->lru_lock, &flags, cc);
 ...
 */
unsigned long isolate_migratepages_range(struct compact_control *cc, unsigned long start_pfn, unsigned long end_pfn)
{
 ...
 *  判断页块的迁移类型
 *
 * 1. 对于异步类型的内存规整，只支持迁移类型为 可移动(MOVABLE)的页块
 * 2. 对于同步模式的内存规整，判断页块迁移类型。
 *          若当前的页块的迁移类型和请求页面的迁移类型不一致，那么对跳过这个页块。
 */
...
{
    return (cc->free_pfn >> pageblock_order) <= (cc->migrate_pfn >> pageblock_order);
}
 ...
 */
static void move_freelist_head(struct list_head *freelist, struct page *freepage)
{
    LIST_HEAD(sublist);
 ...
 *
 */
static void move_freelist_tail(struct list_head *freelist, struct page *freepage)
{
    LIST_HEAD(sublist);
...
static unsigned long fast_isolate_freepages(struct compact_control *cc)
{
    ...
    for (order = cc->search_order; !page && order >= 0; order = next_search_order(cc, order)) {
...
static void isolate_freepages(struct compact_control *cc)
{
    ...
    for (; block_start_pfn >= low_pfn;
         block_end_pfn = block_start_pfn, block_start_pfn -= pageblock_nr_pages, isolate_start_pfn = block_start_pfn) {
 ...
 *
 * 从 zone的尾部开始查找空闲页面，把空闲页面添加到  freepages 链表里
 */
...
int __read_mostly sysctl_compact_unevictable_allowed = 0;
#else
int __read_mostly sysctl_compact_unevictable_allowed = 1;
#endif

static inline void update_fast_start_pfn(struct compact_control *cc, unsigned long pfn)
{
...
static inline unsigned long reinit_migrate_pfn(struct compact_control *cc)
{
...
static unsigned long fast_find_migrateblock(struct compact_control *cc)
{
     ...
     *  从 order 最大  开始
     */
    for (order = cc->order - 1; order >= PAGE_ALLOC_COSTLY_ORDER && pfn == cc->migrate_pfn && nr_scanned < limit;
         order--) {
        /**
         *  空闲 area
         */
 ...
 * compact_control.
 *
 *  扫描 并且寻觅 zone 中可迁移的页面可迁移的页面被添加到 cc->migratepages 链表中
 */
static isolate_migrate_t isolate_migratepages(struct compact_control *cc)
{
     ...
     *  分离模式
     */
    const isolate_mode_t isolate_mode = (sysctl_compact_unevictable_allowed ? ISOLATE_UNEVICTABLE : 0) |
	 ...
	 */
    for (; block_end_pfn <= cc->free_pfn; fast_find_block = false, low_pfn = block_end_pfn,
		 ...
		 */
        if (IS_ALIGNED(low_pfn, pageblock_nr_pages) && !fast_find_block && !isolation_suitable(cc, page))
            continue;
...
static unsigned int fragmentation_score_zone(struct zone *zone)
{
    ...
    score = zone->present_pages * extfrag_for_order(zone, COMPACTION_HPAGE_ORDER);
 ...
 *  内存规整结束的条件有两个：
 *
 *  1. cc->migrate_pfn 和 cc->free_pfn 两个指针相遇, 见`compact_scanners_met()`
 *  2. 判断 zone 里面 order 对应的迁移类型的空闲链表是否有成员，
 *      zone->free_area[order].free_list[MIGRATE_MOVABLE]
 *
 *  若对应 的迁移类型总的空闲链表没有空闲对象，那么假设 可以从其他迁移类型中"借"一些空闲页面过来
 *      见`find_suitable_fallback()`
 */
static enum compact_result __compact_finished(struct compact_control *cc)
{
    ...
    for (order = cc->order; order < MAX_ORDER; order++) {
        ...
        if (find_suitable_fallback(area, order, migratetype, true, &can_steal) != -1) {
			 ...
			 */
            if (cc->mode == MIGRATE_ASYNC || IS_ALIGNED(cc->migrate_pfn, pageblock_nr_pages)) {
                return COMPACT_SUCCESS;
            }
 ...
 */
static enum compact_result __compaction_suitable(struct zone *zone, int order, unsigned int alloc_flags,
                                                 int highest_zoneidx, unsigned long wmark_target)
{
	 ...
	 *  order <=3 : 使用最低警戒水位
	 */
    watermark = (order > PAGE_ALLOC_COSTLY_ORDER /* 3 */) ? low_wmark_pages(zone) : min_wmark_pages(zone);
     ...
     *  再次检查水位
     */
    if (!__zone_watermark_ok(zone, 0, watermark, highest_zoneidx, ALLOC_CMA, wmark_target))
        /**
         *  不满足内存规整的条件
         */
...
enum compact_result compaction_suitable(struct zone *zone, int order, unsigned int alloc_flags, int highest_zoneidx)
{
     ...
     *  出 2 order 次幂 个物理页面，判断函数为 zone_watermark_ok()
     */
    ret = __compaction_suitable(zone, order, alloc_flags, highest_zoneidx, zone_page_state(zone, NR_FREE_PAGES));
    /*
...
bool compaction_zonelist_suitable(struct alloc_context *ac, int order, int alloc_flags)
{
    struct zone *zone;
    struct zoneref *z;
    ...
    for_each_zone_zonelist_nodemask(zone, z, ac->zonelist, ac->highest_zoneidx, ac->nodemask)
    {
        ...
        compact_result = __compaction_suitable(zone, order, alloc_flags, ac->highest_zoneidx, available);
...
static enum compact_result compact_zone(struct compact_control *cc, struct capture_control *capc)
{
     ...
     *  出 2 order 次幂 个物理页面，判断函数为 zone_watermark_ok()
     */
    ret = compaction_suitable(cc->zone, cc->order, cc->alloc_flags, cc->highest_zoneidx);
	 ...
	 */
    update_cached = !sync && cc->zone->compact_cached_migrate_pfn[0] == cc->zone->compact_cached_migrate_pfn[1];
    ...
    while ((ret = compact_finished(cc)) == COMPACT_CONTINUE) {
         ...
         *  扫描 并且寻觅 zone 中可迁移的页面可迁移的页面被添加到 cc->migratepages 链表中
         */
        switch (isolate_migratepages(cc)) { /* 隔离将被迁移页 */
         ...
         */
            case ISOLATE_NONE:
                if (update_cached) {
                    cc->zone->compact_cached_migrate_pfn[1] = cc->zone->compact_cached_migrate_pfn[0];
                }
         ...
         *  迁移页面的核心函数
         */
        err = migrate_pages(&cc->migratepages, compaction_alloc, /* 从 zone的尾部开始查找空闲页面 */
    ...
    check_drain:
        ...
        if (cc->order > 0 && last_migrated_pfn) {
            unsigned long current_block_start = block_start_pfn(cc->migrate_pfn, cc->order);
...
/**
 *  初始化 内存规整内部使用的数据结构, 并进行内存规整
 */
static enum compact_result compact_zone_order(struct zone *zone, int order, gfp_t gfp_mask, enum compact_priority prio,
                                              unsigned int alloc_flags, int highest_zoneidx, struct page **capture)
{
    enum compact_result ret;
     ...
     *
     */
    struct compact_control cc = { cc.order = order,
                                  cc.search_order = order,
                                  cc.gfp_mask = gfp_mask,
                                  cc.zone = zone,
                                  cc.mode = (prio == COMPACT_PRIO_ASYNC) ? MIGRATE_ASYNC : MIGRATE_SYNC_LIGHT,
 ...
 *
 */
enum compact_result try_to_compact_pages(gfp_t gfp_mask, unsigned int order, unsigned int alloc_flags,
                                         ...
                                         struct page **capture)
{
    ...
    for_each_zone_zonelist_nodemask(zone, z, ac->zonelist, ac->highest_zoneidx, ac->nodemask)
    {
        ...
        if (prio != COMPACT_PRIO_ASYNC && (status == COMPACT_COMPLETE || status == COMPACT_PARTIAL_SKIPPED))
            /*
		 ...
		 */
        if ((prio == COMPACT_PRIO_ASYNC && need_resched()) || fatal_signal_pending(current))
            break;
    }
 ...
 * /proc/sys/vm/compact_memory
 */
int sysctl_compaction_handler(struct ctl_table *table, int write, void *buffer, size_t *length, loff_t *ppos)
{
    if (write)
        compact_nodes();
...
static ssize_t sysfs_compact_node(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int nid = dev->id;
...
static bool kcompactd_node_suitable(pg_data_t *pgdat)
{
    ...
    for (zoneid = 0; zoneid <= highest_zoneidx; zoneid++) {
        ...
        if (compaction_suitable(zone, pgdat->kcompactd_max_order, 0, highest_zoneidx) == COMPACT_CONTINUE)
            return true;
    }
...
void wakeup_kcompactd(pg_data_t *pgdat, int order, int highest_zoneidx)
{
	 ...
	 *
	 */
    trace_mm_compaction_wakeup_kcompactd(pgdat->node_id, order, highest_zoneidx);
    /**
     *  唤醒
     */
...
static int kcompactd(void *p /* 内存节点-所有的ZONE */)
{
    ...
    while (!kthread_should_stop()) { /* 不应该停止 */
         ...
         *
         */
        if (wait_event_freezable_timeout(pgdat->kcompactd_wait, kcompactd_work_requested(pgdat),
        ...
        if (should_proactive_compact_node(pgdat)) {
			 ...
			 */
            proactive_defer = score < prev_score ? 0 : 1 << COMPACT_MAX_DEFER_SHIFT;
        }
    }
...
static int __init kcompactd_init(void)
{
    ...
    ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "mm/compaction:online", kcompactd_cpu_online, NULL);
    if (ret < 0) {
