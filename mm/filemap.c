...
static void page_cache_delete_batch(struct address_space *mapping, struct pagevec *pvec)
{
    ...
    xas_for_each(&xas, page, ULONG_MAX)
    {
        ...
        if (page != pvec->pages[i]) {
            VM_BUG_ON_PAGE(page->index > pvec->pages[i]->index, page);
            continue;
        }
...
int file_check_and_advance_wb_err(struct file *file)
{
    ...
    if (errseq_check(&mapping->wb_err, old)) {
        ...
        old = file->f_wb_err;
        err = errseq_check_and_advance(&mapping->wb_err, &file->f_wb_err);
 ...
 * @return noinline
 */
noinline int __add_to_page_cache_locked(struct page *page, struct address_space *mapping, pgoff_t offset, gfp_t gfp,
                                        void **shadowp)
{
    /**
     *
...
static inline int wait_on_page_bit_common(wait_queue_head_t *q, struct page *page, int bit_nr, int state,
...
static int __wait_on_page_locked_async(struct page *page, struct wait_page_queue *wait, bool set)
{
...
ssize_t generic_file_buffered_read(struct kiocb *iocb, struct iov_iter *iter, ssize_t written)
{
    ...
    find_page:
        ...
        if (!PageUptodate(page)) {
            ...
            if (inode->i_blkbits == PAGE_SHIFT || !mapping->a_ops->is_partially_uptodate)
...
ssize_t generic_file_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
    ...
    if (iocb->ki_flags & IOCB_DIRECT) {
        ...
        if (iocb->ki_flags & IOCB_NOWAIT) {
             ...
             *
             */
            if (filemap_range_has_page(mapping, iocb->ki_pos, iocb->ki_pos + count - 1))
                return -EAGAIN;
        } else {
 ...
 */
static int lock_page_maybe_drop_mmap(struct vm_fault *vmf, struct page *page, struct file **fpin)
{
    if (trylock_page(page))
        return 1;
...
void filemap_map_pages(struct vm_fault *vmf, pgoff_t start_pgoff, pgoff_t end_pgoff)
{
    ...
    xas_for_each(&xas, head, end_pgoff)
    {
        ...
        if (!PageUptodate(head) || PageReadahead(page) || PageHWPoison(page))
            goto skip;
...
static struct page *do_read_cache_page(struct address_space *mapping, pgoff_t index,
                                       int (*filler)(void *, struct page *), void *data, gfp_t gfp)
{
    struct page *page;
    int err;
repeat:
...
int pagecache_write_begin(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned flags,
...
int pagecache_write_end(struct file *file, struct address_space *mapping, loff_t pos, unsigned len, unsigned copied,
...
ssize_t generic_file_direct_write(struct kiocb *iocb, struct iov_iter *from)
{
    ...
    if (iocb->ki_flags & IOCB_NOWAIT) {
        ...
        if (filemap_range_has_page(inode->i_mapping, pos, pos + write_len - 1))
            return -EAGAIN;
    } else {
...
ssize_t generic_perform_write(struct file *file, struct iov_iter *i, loff_t pos)
{
    ...
    again:
        ...
        if (unlikely(copied == 0)) {
			 ...
			 */
            bytes = min_t(unsigned long, PAGE_SIZE - offset, iov_iter_single_seg_count(i));
            goto again;
        }
        pos += copied;
        written += copied;
 ...
 *
 * 释放 page持有的与页面相关的元数据，通常存储在private字段中
 */
