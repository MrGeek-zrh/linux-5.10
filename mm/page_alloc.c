...
volatile unsigned long __latent_entropy latent_entropy;
...
nodemask_t __read_mostly node_states[NR_NODE_STATES] = {
    ...
    [N_HIGH_MEMORY] = { { [0] = 1UL } },
#endif
    [N_MEMORY] = { { [0] = 1UL } },        [N_CPU] = { { [0] = 1UL } },
#endif /* NUMA */
};
EXPORT_SYMBOL(node_states);

atomic_long_t __read_mostly _totalram_pages;
EXPORT_SYMBOL(_totalram_pages);
unsigned long __read_mostly totalreserve_pages;
 ...
 *  最高水位和最低水位
 */
static unsigned long __initdata arch_zone_lowest_possible_pfn[MAX_NR_ZONES];
static unsigned long __initdata arch_zone_highest_possible_pfn[MAX_NR_ZONES];
static unsigned long __initdata required_kernelcore;
static unsigned long __initdata required_kernelcore_percent;
static unsigned long __initdata required_movablecore;
static unsigned long __initdata required_movablecore_percent;
static unsigned long __initdata zone_movable_pfn[MAX_NUMNODES];
static bool __meminitdata mirrored_kernelcore;
 ...
 *  NODE 数
 */
unsigned int __read_mostly nr_node_ids = MAX_NUMNODES;
unsigned int __read_mostly nr_online_nodes = 1;
EXPORT_SYMBOL(nr_node_ids);
EXPORT_SYMBOL(nr_online_nodes);
#endif

int __read_mostly page_group_by_mobility_disabled;
...
static inline bool __meminit early_page_uninitialised(unsigned long pfn)
{
    ...
    if (node_online(nid) && pfn >= NODE_DATA(nid)->first_deferred_pfn /* 第一个需要初始化的 PFN */)
        return true;
 ...
 *
 * 返回true，当剩下的初始化应该被延迟，知道启动周期可以被并行化之后。
 * TODO 2021年7月23日16:28:57
 */
 ...
 */
static __always_inline unsigned long __get_pfnblock_flags_mask(struct page *page, unsigned long pfn, unsigned long mask)
{
...
/* 页帧号块 */
unsigned long get_pfnblock_flags_mask(struct page *page, unsigned long pfn, unsigned long mask)
{
...
static int page_outside_zone_boundaries(struct zone *zone, struct page *page)
{
    ...
    if (ret)
        pr_err("page 0x%lx outside node %d zone %s [ 0x%lx - 0x%lx ]\n", pfn, zone_to_nid(zone), zone->name, start_pfn,
               start_pfn + sp);
...
bool __read_mostly _debug_pagealloc_enabled_early = IS_ENABLED(CONFIG_DEBUG_PAGEALLOC_ENABLE_DEFAULT);
...
static inline struct capture_control *task_capc(struct zone *zone)
{
    ...
    return unlikely(capc) && !(current->flags & PF_KTHREAD) && !capc->page && capc->cc->zone == zone ? capc : NULL;
}

static inline bool compaction_capture(struct capture_control *capc, struct page *page, int order, int migratetype)
{
 ...
 */
static inline bool buddy_merge_likely(unsigned long pfn, unsigned long buddy_pfn, struct page *page, unsigned int order)
{
    ...
    return pfn_valid_within(buddy_pfn) && page_is_buddy(higher_page, higher_buddy, order + 1);
}
 ...
 * 伙伴系统的 页 释放 函数
 */
static inline void __free_one_page(struct page *page, unsigned long pfn, struct zone *zone, unsigned int order,
 ...
 *  释放页
 */
void __free_pages_core(struct page *page, unsigned int order) /* 释放 page */
{
...
void __init memblock_free_pages(struct page *page, unsigned long pfn, /* 释放page */
                                unsigned int order)
{
    if (early_page_uninitialised(pfn) /* 这个PFN是不是在NODE内 */)
        return;

    /**
     *  释放 page
     */
    __free_pages_core(page, order); /* 释放 */
}
...
void set_zone_contiguous(struct zone *zone)
{
    ...
    for (; block_start_pfn < zone_end_pfn(zone); block_start_pfn = block_end_pfn, block_end_pfn += pageblock_nr_pages) {
        block_end_pfn = min(block_end_pfn, zone_end_pfn(zone));

        if (!__pageblock_pfn_to_page(block_start_pfn, block_end_pfn, zone))
            return;
        cond_resched();
    }
...
static atomic_t __initdata pgdat_init_n_undone;
 ...
 */
static void __init deferred_free_pages(unsigned long pfn, unsigned long end_pfn)
{
 ...
 */
static unsigned long __init deferred_init_pages(struct zone *zone, unsigned long pfn, unsigned long end_pfn)
{
    unsigned long nr_pgmask = pageblock_nr_pages - 1;
 ...
 */
static bool __init deferred_init_mem_pfn_range_in_zone(u64 *i, struct zone *zone, unsigned long *spfn,
                                                       unsigned long *epfn, unsigned long first_init_pfn)
{
    u64 j;
 ...
 */
static unsigned long __init deferred_init_maxorder(u64 *i, struct zone *zone, unsigned long *start_pfn,
...
static void __init deferred_init_memmap_chunk(unsigned long start_pfn, unsigned long end_pfn, void *arg)
{
...
__weak int __init deferred_page_init_max_threads(const struct cpumask *node_cpumask)
{
    return 1;
}
...
static int __init deferred_init_memmap(void *data)
{
    ...
    if (!deferred_init_mem_pfn_range_in_zone(&i, zone, &spfn, &epfn, first_init_pfn))
        goto zone_empty;
    ...
    while (spfn < epfn) {
        ...
        deferred_init_mem_pfn_range_in_zone(&i, zone, &spfn, &epfn, epfn_align);
    }
zone_empty:
...
void __init page_alloc_init_late(void)
{
	 ...
	 */
    for_each_populated_zone(zone) zone_pcp_update(zone);
    ...
    for_each_node_state(nid, N_MEMORY) shuffle_free_memory(NODE_DATA(nid));

    for_each_populated_zone(zone) set_zone_contiguous(zone);
}
...
static inline int check_new_page(struct page *page)
{
     ...
     *
     */
    if (likely(page_expected_state(page, PAGE_FLAGS_CHECK_AT_PREP | __PG_HWPOISON)))
        return 0;
...
{
    return (IS_ENABLED(CONFIG_PAGE_POISONING_ZERO) && page_poisoning_enabled()) || want_init_on_free();
}
...
static bool can_steal_fallback(unsigned int order, int start_mt)
{
     ...
     *
     */
    if (order >= pageblock_order / 2 || start_mt == MIGRATE_RECLAIMABLE || start_mt == MIGRATE_UNMOVABLE ||
...
void __putback_isolated_page(struct page *page, unsigned int order, int mt)
{
    ...
    __free_one_page(page, page_to_pfn(page), zone, order, mt, FPI_SKIP_REPORT_NOTIFY | FPI_TO_TAIL);
}
...
static struct page *__rmqueue_pcplist(struct zone *zone, int migratetype, unsigned int alloc_flags,
                                      struct per_cpu_pages *pcp, struct list_head *list)
{
    ...
    do {
        ...
        if (list_empty(list)) {
            ...
            pcp->count += rmqueue_bulk(zone, 0, pcp->batch, list, migratetype, alloc_flags);
...
static struct page *rmqueue_pcplist(struct zone *preferred_zone, struct zone *zone, gfp_t gfp_flags, int migratetype,
                                    unsigned int alloc_flags)
{
    struct per_cpu_pages *pcp;
    struct list_head *list;
 ...
 */
static inline struct page *rmqueue(struct zone *preferred_zone, struct zone *zone, unsigned int order, gfp_t gfp_flags,
                                   unsigned int alloc_flags, int migratetype)
{
    ...
    if (likely(order == 0)) {
        ...
        if (!IS_ENABLED(CONFIG_CMA) || alloc_flags & ALLOC_CMA || migratetype != MIGRATE_MOVABLE) {
             ...
             *  pcp: Per CPU Page
             *
             *  如果这个 pcp 的链表中没有 free page，将使用 __rmqueue
             *  从伙伴系统申请 pcp->batch 个页面加入链表中
             *  pcp从 伙伴系统申请 过程是慢速过程。
             */
 ...
 * @return false
 */
static inline bool zone_watermark_fast(struct zone *z, unsigned int order, unsigned long mark, int highest_zoneidx,
 ...
 *  判定 连续内存管理
 */
static inline unsigned int current_alloc_flags(gfp_t gfp_mask, unsigned int alloc_flags)
{
...
retry:
    ...
    for_next_zone_zonelist_nodemask(zone, z, ac->highest_zoneidx, ac->nodemask)
    {
        ...
        if (!zone_watermark_fast(zone, order, mark, ac->highest_zoneidx, alloc_flags, gfp_mask)) {
            ...
            switch (ret) {
                ...
                default:
                    /**
				 *  did we reclaim enough - 是否回收的够用了？
				 *  如果够用了，那么直接 跳转到 try_this_zone 进行分配
				 */
    ...
    try_this_zone:
         ...
         *  从已知 NODE 中分配 page
         *  1. 剩余内存够用，直接分配
         *  2. 剩余内存够用，但是分配后进入 low 水位，唤醒 kswapd 进行异步回收
         */
...
void warn_alloc(gfp_t gfp_mask, nodemask_t *nodemask, const char *fmt, ...)
{
    ...
    vaf.fmt = fmt;
    vaf.va = &args;
    pr_warn("%s: %pV, mode:%#x(%pGg), nodemask=%*pbl", current->comm, &vaf, gfp_mask, &gfp_mask,
...
static inline struct page *__alloc_pages_cpuset_fallback(gfp_t gfp_mask, unsigned int order, unsigned int alloc_flags,
                                                         const struct alloc_context *ac)
{
    struct page *page;
 ...
 */
static inline struct page *__alloc_pages_may_oom(gfp_t gfp_mask, unsigned int order, const struct alloc_context *ac,
...
static struct page *__alloc_pages_direct_compact(gfp_t gfp_mask, unsigned int order, unsigned int alloc_flags,
...
static inline bool should_compact_retry(struct alloc_context *ac, int order, int alloc_flags,
                                        enum compact_result _compact_result, enum compact_priority *compact_priority,
...
__alloc_pages_direct_reclaim(gfp_t gfp_mask, unsigned int order, unsigned int alloc_flags,
...
/**
 *  唤醒 所有 kswapd 进程（kswapd 一个 node 一个）
 */
 ...
 *  是否需要重试 直接页面回收
 */
static inline bool should_reclaim_retry(gfp_t gfp_mask, unsigned order, struct alloc_context *ac, int alloc_flags,
                                        bool did_some_progress, int *no_progress_loops)
{
    ...
    for_each_zone_zonelist_nodemask(zone, z, ac->zonelist, ac->highest_zoneidx, ac->nodemask)
    {
		 ...
		 * reclaimable pages?
		 */
        wmark = __zone_watermark_ok(zone, order, min_wmark, ac->highest_zoneidx, alloc_flags, available);
...
static inline bool check_retry_cpuset(int cpuset_mems_cookie, struct alloc_context *ac)
{
	 ...
	 */
    if (cpusets_enabled() && ac->nodemask && !cpuset_nodemask_valid_mems_allowed(ac->nodemask)) {
        ac->nodemask = NULL;
        return true;
    }
...
retry_cpuset:
	 ...
	 *
	 *  重新计算首选推荐的 zone，因为可能在快速路径修改了内存节点掩码，
	 *  或 使用 cpuset 机制做了修改
	 */
	 ...
	 *  3. 不能访问系统预留内存
	 */
    if (can_direct_reclaim && (costly_order || (order > 0 && ac->migratetype != MIGRATE_MOVABLE)) &&
         ...
         * @compact_result  内存规整结果
         */
        page = __alloc_pages_direct_compact(gfp_mask, order, alloc_flags, ac, INIT_COMPACT_PRIORITY, &_compact_result);
        if (page)
            goto got_pg;
        ...
        if (costly_order && (gfp_mask & __GFP_NORETRY)) {
			 ...
			 *    own.
			 */
            if (_compact_result == COMPACT_SKIPPED || _compact_result == COMPACT_DEFERRED)
                goto nopage;
...
retry:
    ...
    page = __alloc_pages_direct_compact(gfp_mask, order, alloc_flags, ac, compact_priority, &_compact_result);
    if (page)
        goto got_pg;
    ...
    if (did_some_progress > 0 &&
        should_compact_retry(ac, order, alloc_flags, _compact_result, &compact_priority, &compaction_retries))
        goto retry;
 ...
 * @return false
 */
static inline bool prepare_alloc_pages(gfp_t gfp_mask, unsigned int order, int preferred_nid, nodemask_t *nodemask,
...
out:
     ...
     */
    if (memcg_kmem_enabled() /* memory cgroup */ && (gfp_mask & __GFP_ACCOUNT) && page &&
...
static struct page *__page_frag_cache_refill(struct page_frag_cache *nc, gfp_t gfp_mask)
{
    ...
    gfp_mask |= __GFP_COMP | __GFP_NOWARN | __GFP_NORETRY | __GFP_NOMEMALLOC;
    page = alloc_pages_node(NUMA_NO_NODE, gfp_mask, PAGE_FRAG_CACHE_MAX_ORDER);
...
long si_mem_available(void)
{
	 ...
	 */
    reclaimable =
            global_node_page_state_pages(NR_SLAB_RECLAIMABLE_B) + global_node_page_state(NR_KERNEL_MISC_RECLAIMABLE);
...
void si_meminfo(struct sysinfo *val) /*系统信息  */
{
...
static void show_migration_types(unsigned char type)
{
    static const char types[MIGRATE_TYPES] = {
        [MIGRATE_UNMOVABLE] = 'U', [MIGRATE_MOVABLE] = 'M', [MIGRATE_RECLAIMABLE] = 'E', [MIGRATE_HIGHATOMIC] = 'H',
#ifdef CONFIG_CMA
        [MIGRATE_CMA] = 'C',
#endif
#ifdef CONFIG_MEMORY_ISOLATION
...
void show_free_areas(unsigned int filter, nodemask_t *nodemask)
{
    ...
    printk("active_anon:%lu inactive_anon:%lu isolated_anon:%lu\n"
           ...
           global_node_page_state(NR_ACTIVE_ANON), global_node_page_state(NR_INACTIVE_ANON),
           global_node_page_state(NR_ISOLATED_ANON), global_node_page_state(NR_ACTIVE_FILE),
           global_node_page_state(NR_INACTIVE_FILE), global_node_page_state(NR_ISOLATED_FILE),
           global_node_page_state(NR_UNEVICTABLE), global_node_page_state(NR_FILE_DIRTY),
           global_node_page_state(NR_WRITEBACK), global_node_page_state_pages(NR_SLAB_RECLAIMABLE_B),
           global_node_page_state_pages(NR_SLAB_UNRECLAIMABLE_B), global_node_page_state(NR_FILE_MAPPED),
           global_node_page_state(NR_SHMEM), global_zone_page_state(NR_PAGETABLE), global_zone_page_state(NR_BOUNCE),
           global_zone_page_state(NR_FREE_PAGES), free_pcp, global_zone_page_state(NR_FREE_CMA_PAGES));
    ...
    for_each_online_pgdat(pgdat)
    {
        ...
        printk("Node %d"
               ...
               K(node_page_state(pgdat, NR_ACTIVE_FILE)), K(node_page_state(pgdat, NR_INACTIVE_FILE)),
               K(node_page_state(pgdat, NR_UNEVICTABLE)), K(node_page_state(pgdat, NR_ISOLATED_ANON)),
               K(node_page_state(pgdat, NR_ISOLATED_FILE)), K(node_page_state(pgdat, NR_FILE_MAPPED)),
               K(node_page_state(pgdat, NR_FILE_DIRTY)), K(node_page_state(pgdat, NR_WRITEBACK)),
...
#endif
               K(node_page_state(pgdat, NR_WRITEBACK_TEMP)), node_page_state(pgdat, NR_KERNEL_STACK_KB),
#ifdef CONFIG_SHADOW_CALL_STACK
    ...
    for_each_populated_zone(zone)
    {
        ...
        free_pcp = 0;
        for_each_online_cpu(cpu) free_pcp += per_cpu_ptr(zone->pageset, cpu)->pcp.count;
        ...
        printk(KERN_CONT "%s"
                         ...
                         " free_cma:%lukB"
                         "\n",
               zone->name, K(zone_page_state(zone, NR_FREE_PAGES)), K(min_wmark_pages(zone)), K(low_wmark_pages(zone)),
               K(high_wmark_pages(zone)), K(zone->nr_reserved_highatomic),
               K(zone_page_state(zone, NR_ZONE_ACTIVE_ANON)), K(zone_page_state(zone, NR_ZONE_INACTIVE_ANON)),
               K(zone_page_state(zone, NR_ZONE_ACTIVE_FILE)), K(zone_page_state(zone, NR_ZONE_INACTIVE_FILE)),
               K(zone_page_state(zone, NR_ZONE_UNEVICTABLE)), K(zone_page_state(zone, NR_ZONE_WRITE_PENDING)),
               K(zone->present_pages), K(zone_managed_pages(zone)), K(zone_page_state(zone, NR_MLOCK)),
               K(zone_page_state(zone, NR_PAGETABLE)), K(zone_page_state(zone, NR_BOUNCE)), K(free_pcp),
               K(this_cpu_read(zone->pageset->pcp.count)), K(zone_page_state(zone, NR_FREE_CMA_PAGES)));
...
int local_memory_node(int node)
{
    ...
    z = first_zones_zonelist(node_zonelist(node, GFP_KERNEL), gfp_zone(GFP_KERNEL), NULL);
...
static bool __meminit overlap_memmap_init(unsigned long zone, unsigned long *pfn)
{
    ...
    /**
     *  一般 moveable 不太会发生好像 2021年7月23日16:27:14
     */
    if (mirrored_kernelcore && zone == ZONE_MOVABLE) {
        ...
        if (*pfn >= memblock_region_memory_base_pfn(r) && memblock_is_mirror(r)) {
 ...
 */
/**
 * 初始化一个zone的memmap,将范围内的所有page添加到zone
 * @param size: memmap大小,需要初始化的页数
 * @param nid: NUMA节点号
 * @param zone: zone类型(zone index)
 * @param start_pfn: 起始页帧号
 * @param zone_end_pfn: zone结束页帧号(不包含) 
 * @param context: 初始化上下文,表明是早期初始化还是热插拔
 * @param altmap: 备用记忆体映射(一般为NULL)  
 * @param migratetype: 内存迁移类型(MIGRATE_MOVABLE等)
 */
void __meminit memmap_init_zone(unsigned long size, int nid, unsigned long zone, unsigned long start_pfn,
                                unsigned long zone_end_pfn, enum meminit_context context, struct vmem_altmap *altmap,
                                int migratetype)
{
    unsigned long pfn, end_pfn = start_pfn + size; // 计算结束pfn
    struct page *page; // 保存需要初始化的页实例

    // 记录最大memmap页帧号,用于后续扩展
...
#ifdef CONFIG_ZONE_DEVICE
void __ref memmap_init_zone_device(struct zone *zone, unsigned long start_pfn, unsigned long nr_pages,
...
static int zone_batchsize(struct zone *zone)
{
	 ...
	 * size of the zone.
	 *
	 * per-CPU pages 池，取 所有管理 页 的  1/1024
	 */
    batch = zone_managed_pages(zone) / 1024;

    //以我的两个环境为例，只给出 整个 NODE管理的页数
    //*  我的虚拟机 是8GB
 ...
 */
void __init setup_per_cpu_pageset(void) /* 为每个 CPU 分配 */
{
...
static __meminit void zone_pcp_init(struct zone *zone)
{
    ...
    //[    0.000000]   DMA32 zone: 432195 pages, LIFO batch:31
...
{
    struct pglist_data *pgdat = zone->zone_pgdat;
    int rtoax_zone_idx = zone_idx(zone) + 1;

    if (rtoax_zone_idx > pgdat->nr_zones)
        pgdat->nr_zones = rtoax_zone_idx;

    zone->zone_start_pfn = zone_start_pfn;

    mminit_dprintk(MMINIT_TRACE, "memmap_init", "Initialising map node %d zone %lu pfns %lu -> %lu\n", pgdat->node_id,
                   (unsigned long)zone_idx(zone), zone_start_pfn, (zone_start_pfn + size));
 ...
 * PFNs will be 0.
 */
void __init get_pfn_range_for_nid(unsigned int nid, unsigned long *start_pfn, unsigned long *end_pfn)
{
    ...
    *end_pfn = 0;
    ...
    /**
     *  遍历 memblock.memory 找到对应的 nid， 然后获取 start 和 end pfn
     */
...
static void __init find_usable_zone_for_movable(void)
{
    ...
    for (zone_index = MAX_NR_ZONES - 1; zone_index >= 0; zone_index--) {
        ...
        if (arch_zone_highest_possible_pfn[zone_index] > arch_zone_lowest_possible_pfn[zone_index])
            break;
    }
 ...
 */
static void __init adjust_zone_range_for_zone_movable(int nid, unsigned long zone_type, unsigned long node_start_pfn,
                                                      ...
                                                      unsigned long *zone_end_pfn)
{
    ...
    if (zone_movable_pfn[nid]) {
        ...
        } else if (!mirrored_kernelcore && *zone_start_pfn < zone_movable_pfn[nid] &&
 ...
 *  present_pages = zone_spanned_pages_in_node() - zone_absent_pages_in_node()
 */
static unsigned long __init zone_spanned_pages_in_node(int nid, unsigned long zone_type, unsigned long node_start_pfn,
                                                       unsigned long node_end_pfn, unsigned long *zone_start_pfn,
                                                       unsigned long *zone_end_pfn)
{
    /**
 ...
 */
unsigned long __init __absent_pages_in_range(int nid, unsigned long range_start_pfn, unsigned long range_end_pfn)
{
 ...
 *  返回节点上某个区域的孔中的页框数
 */
static unsigned long __init zone_absent_pages_in_node(int nid, unsigned long zone_type, unsigned long node_start_pfn,
 ...
 * [    0.000000] On node 0 totalpages: 2096926 -> 8GB
 */
static void __init calculate_node_totalpages(struct pglist_data *pgdat, unsigned long node_start_pfn,
                                             unsigned long node_end_pfn)
{
    ...
    for (i = 0; i < MAX_NR_ZONES; i++) {
         ...
         *  present_pages = zone_spanned_pages_in_node() - zone_absent_pages_in_node()
         */
        spanned /*跨越*/ = zone_spanned_pages_in_node(pgdat->node_id, i, node_start_pfn, node_end_pfn, &zone_start_pfn,
                                                      &zone_end_pfn);
        /**
         *  返回节点上某个区域的孔中的页框数
         */
        absent /*缺席的*/ = zone_absent_pages_in_node(pgdat->node_id, i, node_start_pfn, node_end_pfn);
    ...
    //node 0 size: 32423 MB
    //node 0 free: 13516 MB
 ...
 *
 */
static void __ref setup_usemap(struct pglist_data *pgdat, struct zone *zone, unsigned long zone_start_pfn,
...
static unsigned long __init calc_memmap_size(unsigned long spanned_pages, unsigned long present_pages)
{
    ...
    /**
     *  pages 需要的字节数 >> 12 = 需要的页数
     *  也就是说计算需要存储 struct page 一共需要多少个物理页；
     */
...
static void __meminit zone_init_internals(struct zone *zone, enum zone_type idx, int nid, unsigned long remaining_pages)
{
     ...
     *  管理的页面
     *
     * 将减去 管理 页面需要的内存空间后的remainning pages 添加到 zone 管理的页面中
     */
...
static void __init free_area_init_core(struct pglist_data *pgdat)
{
    ...
    for (j = 0; j < MAX_NR_ZONES; j++) {
        ...
        /**
         *  如果不是highmem, x86-64 恒成立
         */
        if (!is_highmem_idx(j)) {
            /**
             *  我觉得这肯定成立，不能说内存还不够方管理数据结构page吧
             */
            if (freesize >= memmap_pages) {
                ...
                //在我的虚拟机上
                //[rongtao@localhost src]$ dmesg | grep "pages use"
                //[    0.000000]   DMA zone: 64 pages used for memmap
                //[    0.000000]   DMA32 zone: 12222 pages used for memmap
                //[    0.000000]   Normal zone: 20480 pages used for memmap
                //
                //在物理服务器上
                //[root@localhost ~]# dmesg | grep "pages use"
                //[    0.000000]   DMA zone: 64 pages used for memmap
                //[    0.000000]   DMA32 zone: 6754 pages used for memmap
             ...
             */
                ...
                pr_warn("  %s zone: %lu pages exceeds(超过) freesize %lu\n", zone_names[j], memmap_pages, freesize);
        }
        ...
        //
        ////在物理服务器上
        //[root@localhost ~]# dmesg | grep "pages reserved"
        //[    0.000000]   DMA zone: 21 pages reserved
		 ...
		 * 并且所有的  highmem pages 将被伙伴系统管理
		 *
		 * 该函数内部会调用 zone_pcp_init() 为 per-CPU-pages 初始化
		 */
...
static void __init init_unavailable_mem(void)
{
    ...
    /**
     *  遍历 memblock.memory 的所有 range
     */
    for_each_mem_range(i, &start, &end)
    {
        /**
         *  next = 上一块的末尾 [start, end)
         */
        if (next < start)
	 ...
	 * state.
	 */
    pgcnt += init_unavailable_range(PFN_DOWN(next), round_up(max_pfn, PAGES_PER_SECTION));
...
static void __init find_zone_movable_pfns_for_nodes(void)
{
    ...
    if (movable_node_is_enabled()) {
        ...
        for_each_mem_region(r)
        {
             ...
             *
             */
            zone_movable_pfn[nid] = zone_movable_pfn[nid] ? min(usable_startpfn, zone_movable_pfn[nid]) :
                                                            usable_startpfn;
        }
    ...
    if (required_kernelcore_percent)
        required_kernelcore = (totalpages * 100 * required_kernelcore_percent) / 10000UL;
    if (required_movablecore_percent)
        required_movablecore = (totalpages * 100 * required_movablecore_percent) / 10000UL;
    ...
    for_each_node_state(nid, N_MEMORY)
    {
        ...
        for_each_mem_pfn_range(i, nid, &start_pfn, &end_pfn, NULL)
        {
            ...
            if (start_pfn < usable_startpfn) {
                ...
                kernelcore_remaining -= min(kernel_pages, kernelcore_remaining);
                required_kernelcore -= min(kernel_pages, required_kernelcore);
			 ...
			 * satisfied
			 */
            required_kernelcore -= min(required_kernelcore, size_pages);
...
out2:
    ...
    for (nid = 0; nid < MAX_NUMNODES; nid++)
        zone_movable_pfn[nid] = roundup(zone_movable_pfn[nid], MAX_ORDER_NR_PAGES);
...
{
    return cmdline_parse_core(p, &required_movablecore, &required_movablecore_percent);
}
...
#undef adj_init_size
    ...
    pr_info("Memory: %luK/%luK available (%luK kernel code, %luK rwdata, %luK rodata, "
            "%luK init, %luK bss, %luK reserved, %luK cma-reserved" #ifdef CONFIG_HIGHMEM
            //		", %luK highmem"\
#endif
            "%s%s)\n",
            nr_free_pages() << (PAGE_SHIFT - 10), physpages << (PAGE_SHIFT - 10), codesize >> 10, datasize >> 10,
            rosize >> 10, (init_data_size + init_code_size) >> 10, bss_size >> 10,
            (physpages - totalram_pages() - totalcma_pages) << (PAGE_SHIFT - 10), totalcma_pages << (PAGE_SHIFT - 10),
#ifdef CONFIG_HIGHMEM
...
static void __setup_per_zone_wmarks(void)
{
    ...
    for_each_zone(zone)
    {
        ...
        spin_lock_irqsave(&zone->lock, flags);
        tmp = (u64)pages_min * zone_managed_pages(zone); /* 获取管理页 */
 ...
 */
void setup_per_zone_wmarks(void) /* 每个 zone 的 watermark */
{
    static DEFINE_SPINLOCK(lock);
 ...
 *	changes.
 */
int min_free_kbytes_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length, loff_t *ppos)
{
    int rc;
...
int watermark_scale_factor_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length,
                                          loff_t *ppos)
{
    int rc;
...
static void setup_min_unmapped_ratio(void)
{
    ...
    for_each_online_pgdat(pgdat) pgdat->min_unmapped_pages = 0;

    for_each_zone(zone)
            zone->zone_pgdat->min_unmapped_pages += (zone_managed_pages(zone) * sysctl_min_unmapped_ratio) / 100;
}

int sysctl_min_unmapped_ratio_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length,
                                             loff_t *ppos)
{
    int rc;
...
static void setup_min_slab_ratio(void)
{
    ...
    for_each_online_pgdat(pgdat) pgdat->min_slab_pages = 0;

    for_each_zone(zone) zone->zone_pgdat->min_slab_pages += (zone_managed_pages(zone) * sysctl_min_slab_ratio) / 100;
}

int sysctl_min_slab_ratio_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length, loff_t *ppos)
{
    int rc;
 ...
 */
int lowmem_reserve_ratio_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length, loff_t *ppos)
{
    int i;
...
static void __zone_pcp_update(struct zone *zone)
{
    ...
    for_each_possible_cpu(cpu) pageset_set_high_and_batch(zone, per_cpu_ptr(zone->pageset, cpu));
}
 ...
 */
int percpu_pagelist_fraction_sysctl_handler(struct ctl_table *table, int write, void *buffer, size_t *length,
                                            loff_t *ppos)
{
    struct zone *zone;
    ...
    if (percpu_pagelist_fraction && percpu_pagelist_fraction < MIN_PERCPU_PAGELIST_FRACTION) {
    ...
    for_each_populated_zone(zone) __zone_pcp_update(zone);
out:
...
void *__init alloc_large_system_hash(const char *tablename, unsigned long bucketsize, unsigned long numentries,
                                     int scale, int flags, unsigned int *_hash_shift, unsigned int *_hash_mask,
                                     unsigned long low_limit, unsigned long high_limit)
{
    ...
    pr_info("%s hash table entries: %ld (order: %d, %lu bytes, %s)\n", tablename, 1UL << log2qty,
            ilog2(size) - PAGE_SHIFT, size, virt ? "vmalloc" : "linear");
...
{
    return pfn & ~(max_t(unsigned long, MAX_ORDER_NR_PAGES, pageblock_nr_pages) - 1);
}

static unsigned long pfn_max_align_up(unsigned long pfn)
{
    return ALIGN(pfn, max_t(unsigned long, MAX_ORDER_NR_PAGES, pageblock_nr_pages));
}
...
static int __alloc_contig_migrate_range(struct compact_control *cc, unsigned long start, unsigned long end)
{
    ...
    while (pfn < end || !list_empty(&cc->migratepages)) {
        ...
        ret = migrate_pages(&cc->migratepages, alloc_migration_target, NULL, (unsigned long)&mtc, cc->mode,
                            MR_CONTIG_RANGE);
    }
    if (ret < 0) {
...
int alloc_contig_range(unsigned long start, unsigned long end, unsigned migratetype, gfp_t gfp_mask)
{
    ...
    ret = start_isolate_page_range(pfn_max_align_down(start), pfn_max_align_up(end), migratetype, 0);
    if (ret)
        return ret;
    ...
    undo_isolate_page_range(pfn_max_align_down(start), pfn_max_align_up(end), migratetype);
    return ret;
}
 ...
 * buddy allocator.
 */
static void break_down_buddy_pages(struct zone *zone, struct page *page, struct page *target, int low, int high,
                                   int migratetype)
{
    unsigned long size = 1 << high;
...
bool take_page_off_buddy(struct page *page)
{
    ...
    for (order = 0; order < MAX_ORDER; order++) {
        ...
        if (PageBuddy(page_head) && page_order >= order) {
            ...
            int migratetype = get_pfnblock_migratetype(page_head, pfn_head);

            del_page_from_free_list(page_head, zone, page_order);
            break_down_buddy_pages(zone, page_head, page, 0, page_order, migratetype);
            ret = true;
            break;
        }
