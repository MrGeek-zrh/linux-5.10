...
static inline void free_pmd_range(struct mmu_gather *tlb, pud_t *pud, unsigned long addr, unsigned long end,
...
static inline void free_pud_range(struct mmu_gather *tlb, p4d_t *p4d, unsigned long addr, unsigned long end,
                                  unsigned long floor, unsigned long ceiling)
{
    pud_t *pud;
    unsigned long next;
...
static inline void free_p4d_range(struct mmu_gather *tlb, pgd_t *pgd, unsigned long addr, unsigned long end,
                                  unsigned long floor, unsigned long ceiling)
{
    p4d_t *p4d;
    unsigned long next;
...
static void print_bad_pte(struct vm_area_struct *vma, unsigned long addr, pte_t pte, struct page *page)
{
    ...
    pr_alert("addr:%px vm_flags:%08lx anon_vma:%px mapping:%px index:%lx\n", (void *)addr, vma->vm_flags, vma->anon_vma,
             mapping, index);
    pr_alert("file:%pD fault:%ps mmap:%ps readpage:%ps\n", vma->vm_file, vma->vm_ops ? vma->vm_ops->fault : NULL,
             vma->vm_file ? vma->vm_file->f_op->mmap : NULL, mapping ? mapping->a_ops->readpage : NULL);
    dump_stack();
 ...
 *
 *
 查找缺页异常地址addr对应页面的 page 数据结构，返回普通页面映射
 * 根据 PTE 来返回普通映射页面的 page 数据结构，这个page 数据结构是肯定存在的。
 *
 * 一些特殊映射的页面，是不会返回 page 数据结构的
 *  若 返回 NULL，说明这是一个特殊映射页面。
 */
...
static unsigned long copy_nonpresent_pte(struct mm_struct *dst_mm, struct mm_struct *src_mm, pte_t *dst_pte,
                                         pte_t *src_pte, struct vm_area_struct *vma, unsigned long addr, int *rss)
{
    ...
    if (likely(!non_swap_entry(entry))) {
		 ...
		 */
        if (is_write_device_private_entry(entry) && is_cow_mapping(vm_flags)) {
 ...
 * lock.
 */
static inline int copy_present_page(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma, pte_t *dst_pte,
                                    pte_t *src_pte, unsigned long addr, int *rss, struct page **prealloc, pte_t pte,
                                    struct page *page)
{
 ...
 */
static inline int copy_present_pte(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma, pte_t *dst_pte,
                                   pte_t *src_pte, unsigned long addr, int *rss, struct page **prealloc)
{
    struct mm_struct *src_mm = src_vma->vm_mm;
    ...
    if (page) {
        ...
        retval = copy_present_page(dst_vma, src_vma, dst_pte, src_pte, addr, rss, prealloc, pte, page);
        if (retval <= 0)
            return retval;
...
static inline struct page *page_copy_prealloc(struct mm_struct *src_mm, struct vm_area_struct *vma, unsigned long addr)
{
    struct page *new_page;
 ...
 * @end  VMA 结束地址
 */
static int copy_pte_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma, pmd_t *dst_pmd,
...
again:
    ...
    do {
        ...
        if (unlikely(!pte_present(*src_pte))) {
            entry.val = copy_nonpresent_pte(dst_mm, src_mm, dst_pte, src_pte, src_vma, addr, rss);
            if (entry.val)
                break;
            progress += 8;
            continue;
                                 ...
                                 pud_t *src_pud, unsigned long addr, unsigned long end)
{
    ...
    do {
        ...
        if (is_swap_pmd(*src_pmd) || pmd_trans_huge(*src_pmd) || pmd_devmap(*src_pmd)) {
            ...
            err = copy_huge_pmd(dst_mm, src_mm, dst_pmd, src_pmd, addr, src_vma);
            if (err == -ENOMEM)
                return -ENOMEM;
            if (!err)
                continue;
...
static inline int copy_pud_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma, p4d_t *dst_p4d,
                                 p4d_t *src_p4d, unsigned long addr, unsigned long end)
{
    ...
    do {
        ...
        if (pud_trans_huge(*src_pud) || pud_devmap(*src_pud)) {
            ...
            err = copy_huge_pud(dst_mm, src_mm, dst_pud, src_pud, addr, src_vma);
            if (err == -ENOMEM)
                return -ENOMEM;
            if (!err)
                continue;
...
static inline int copy_p4d_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma, pgd_t *dst_pgd,
                                 pgd_t *src_pgd, unsigned long addr, unsigned long end)
{
    ...
    do {
        ...
        if (p4d_none_or_clear_bad(src_p4d))
            continue;
        if (copy_pud_range(dst_vma, src_vma, dst_p4d, src_p4d, addr, next))
            return -ENOMEM;
...
int copy_page_range(struct vm_area_struct *dst_vma, struct vm_area_struct *src_vma)
{
    ...
    do {
        ...
        if (pgd_none_or_clear_bad(src_pgd))
            continue;
        if (unlikely(copy_p4d_range(dst_vma, src_vma, dst_pgd, src_pgd, addr, next))) {
            ret = -ENOMEM;
            break;
        }
...
static unsigned long zap_pte_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pmd_t *pmd, unsigned long addr,
    ...
    do {
        ...
        if (pte_present(ptent)) {
            ...
            if (unlikely(details) && page) {
				 ...
				 */
                if (details->check_mapping && details->check_mapping != page_rmapping(page))
                    continue;
            }
...
static inline unsigned long zap_pmd_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pud_t *pud,
...
static inline unsigned long zap_pud_range(struct mmu_gather *tlb, struct vm_area_struct *vma, p4d_t *p4d,
                                          unsigned long addr, unsigned long end, struct zap_details *details)
{
    pud_t *pud;
    unsigned long next;
...
static inline unsigned long zap_p4d_range(struct mmu_gather *tlb, struct vm_area_struct *vma, pgd_t *pgd,
                                          unsigned long addr, unsigned long end, struct zap_details *details)
{
    p4d_t *p4d;
    unsigned long next;
...
void zap_page_range(struct vm_area_struct *vma, unsigned long start, unsigned long size)
{
    ...
    lru_add_drain();
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, vma->vm_mm, start, start + size);
 ...
 */
static void zap_page_range_single(struct vm_area_struct *vma, unsigned long address, unsigned long size,
                                  struct zap_details *details)
{
    struct mmu_notifier_range range;
    struct mmu_gather tlb;
    ...
    lru_add_drain();
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, vma->vm_mm, address, address + size);
...
{
    if (address < vma->vm_start || address + size > vma->vm_end || !(vma->vm_flags & VM_PFNMAP))
        return;
...
static int insert_page_into_pte_locked(struct mm_struct *mm, pte_t *pte, unsigned long addr, struct page *page,
                                       pgprot_t prot)
{
    if (!pte_none(*pte))
        return -EBUSY;
...
#ifdef pte_index
static int insert_page_in_batch_locked(struct mm_struct *mm, pte_t *pte, unsigned long addr, struct page *page,
                                       pgprot_t prot)
{
    int err;
 ...
 */
static int insert_pages(struct vm_area_struct *vma, unsigned long addr, struct page **pages, unsigned long *num,
                        pgprot_t prot)
{
    pmd_t *pmd = NULL;
    pte_t *start_pte, *pte;
...
more:
    ...
    pages_to_write_in_pmd = min_t(unsigned long, remaining_pages_total, PTRS_PER_PTE - pte_index(addr));
    ...
    while (pages_to_write_in_pmd) {
        ...
        for (pte = start_pte; pte_idx < batch_size; ++pte, ++pte_idx) {
            int err = insert_page_in_batch_locked(mm, pte, addr, pages[curr_page_idx], prot);
            if (unlikely(err)) {
 ...
 */
static int __vm_map_pages(struct vm_area_struct *vma, struct page **pages, unsigned long num, unsigned long offset)
{
...
vm_fault_t vmf_insert_pfn_prot(struct vm_area_struct *vma, unsigned long addr, unsigned long pfn, pgprot_t pgprot)
{
    ...
    return insert_pfn(vma, addr, __pfn_to_pfn_t(pfn, PFN_DEV), pgprot, false);
}
...
static vm_fault_t __vm_insert_mixed(struct vm_area_struct *vma, unsigned long addr, pfn_t pfn, pgprot_t pgprot,
                                    bool mkwrite)
{
    int err;
	 ...
	 */
    if (!IS_ENABLED(CONFIG_ARCH_HAS_PTE_SPECIAL) && !pfn_t_devmap(pfn) && pfn_t_valid(pfn)) {
        struct page *page;
 ...
 */
static int remap_pte_range(struct mm_struct *mm, pmd_t *pmd, unsigned long addr, unsigned long end, unsigned long pfn,
                           pgprot_t prot)
{
    pte_t *pte;
    spinlock_t *ptl;
...
static inline int remap_pmd_range(struct mm_struct *mm, pud_t *pud, unsigned long addr, unsigned long end,
...
static inline int remap_pud_range(struct mm_struct *mm, p4d_t *p4d, unsigned long addr, unsigned long end,
                                  unsigned long pfn, pgprot_t prot)
{
    pud_t *pud;
    unsigned long next;
...
static inline int remap_p4d_range(struct mm_struct *mm, pgd_t *pgd, unsigned long addr, unsigned long end,
                                  unsigned long pfn, pgprot_t prot)
{
    p4d_t *p4d;
    unsigned long next;
...
static int apply_to_pte_range(struct mm_struct *mm, pmd_t *pmd, unsigned long addr, unsigned long end, pte_fn_t fn,
                              void *data, bool create, pgtbl_mod_mask *mask)
{
    pte_t *pte;
    int err = 0;
    spinlock_t *ptl;
...
static int apply_to_pmd_range(struct mm_struct *mm, pud_t *pud, unsigned long addr, unsigned long end, pte_fn_t fn,
                              void *data, bool create, pgtbl_mod_mask *mask)
{
    pmd_t *pmd;
    unsigned long next;
...
static int apply_to_pud_range(struct mm_struct *mm, p4d_t *p4d, unsigned long addr, unsigned long end, pte_fn_t fn,
                              void *data, bool create, pgtbl_mod_mask *mask)
{
    pud_t *pud;
    unsigned long next;
    ...
    do {
        ...
        if (create || !pud_none_or_clear_bad(pud)) {
            err = apply_to_pmd_range(mm, pud, addr, next, fn, data, create, mask);
            if (err)
                break;
        }
    } while (pud++, addr = next, addr != end);
...
static int apply_to_p4d_range(struct mm_struct *mm, pgd_t *pgd, unsigned long addr, unsigned long end, pte_fn_t fn,
                              void *data, bool create, pgtbl_mod_mask *mask)
{
    p4d_t *p4d;
    unsigned long next;
    ...
    do {
        ...
        if (create || !p4d_none_or_clear_bad(p4d)) {
            err = apply_to_pud_range(mm, p4d, addr, next, fn, data, create, mask);
            if (err)
                break;
        }
    } while (p4d++, addr = next, addr != end);
...
static int __apply_to_page_range(struct mm_struct *mm, unsigned long addr, unsigned long size, pte_fn_t fn, void *data,
                                 bool create)
{
    pgd_t *pgd;
 ...
 */
int apply_to_existing_page_range(struct mm_struct *mm, unsigned long addr, unsigned long size, pte_fn_t fn, void *data)
{
 ...
 *  将页面内的信息复制到新的页面
 */
static inline bool cow_user_page(struct page *dst, struct page *src, struct vm_fault *vmf)
{
    bool ret;
    void *kaddr;
    void __user *uaddr;
	 ...
	 *
	 * 如果源 page 是 PFN mapping, 没有 page 结构。
	 *  直接从 原始 的 user 地址 拷贝
	 *
	 */
    kaddr = kmap_atomic(dst);
    uaddr = (void __user *)(addr & PAGE_MASK); /* 用户空间地址 */
...
static vm_fault_t do_page_mkwrite(struct vm_fault *vmf)
{
    ...
    if (vmf->vma->vm_file && IS_SWAPFILE(vmf->vma->vm_file->f_mapping->host))
        return VM_FAULT_SIGBUS;
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf)
{
	 ...
	 *  通知 MMU
	 */
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, mm, vmf->address & PAGE_MASK,
 ...
 * 处理 可写的并且共享的 普通映射 页面
 */
static vm_fault_t wp_page_shared(struct vm_fault *vmf) __releases(vmf->ptl)
{
    struct vm_area_struct *vma = vmf->vma;
    vm_fault_t ret = VM_FAULT_WRITE;

    get_page(vmf->page); /* 引用计数 */
...
static void unmap_mapping_range_vma(struct vm_area_struct *vma, unsigned long start_addr, unsigned long end_addr,
                                    struct zap_details *details)
{
...
static inline void unmap_mapping_range_tree(struct rb_root_cached *root, struct zap_details *details)
{
    ...
    vma_interval_tree_foreach(vma, root, details->first_index, details->last_index)
    {
        vba = vma->vm_pgoff;
        ...
        unmap_mapping_range_vma(vma, ((zba - vba) << PAGE_SHIFT) + vma->vm_start,
                                ((zea - vba + 1) << PAGE_SHIFT) + vma->vm_start, details);
    }
}
...
void unmap_mapping_range(struct address_space *mapping, loff_t const holebegin, loff_t const holelen, int even_cows)
{
    ...
    if (sizeof(holelen) > sizeof(hlen)) {
        long long holeend = (holebegin + holelen + PAGE_SIZE - 1) >> PAGE_SHIFT;
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	 ...
	 */
    if (unlikely((!PageSwapCache(page) || page_private(page) != entry.val)) && swapcache)
        goto out_page;
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	 ...
	 */
    if (!(vmf->flags & FAULT_FLAG_WRITE) /* 没有写标志 */ && !mm_forbids_zeropage(vma->vm_mm) /* 没有禁止 零页 */) {
        /**
		 *  鸡贼的内核，直接返回零页
		 */
        /* 设置 特殊标志位，标志是个 系统零页 */
        ...
        if (!pte_none(*vmf->pte)) {
            /* 更新 TLB -  */
            update_mmu_tlb(vma, vmf->address, vmf->pte); /* 默认为 空 */
            goto unlock;
        }
...
static vm_fault_t __do_fault(struct vm_fault *vmf)
{
    ...
    if (pmd_none(*vmf->pmd) && !vmf->prealloc_pte) {
        vmf->prealloc_pte = pte_alloc_one(vma->vm_mm); /* 分配一个 页表项 */
        if (!vmf->prealloc_pte)
            return VM_FAULT_OOM;
        smp_wmb(); /* See comment in __pte_alloc() */
    }
...
static unsigned long __read_mostly fault_around_bytes = rounddown_pow_of_two(65536);
...
{
    debugfs_create_file_unsafe("fault_around_bytes", 0644, NULL, NULL, &fault_around_bytes_fops);
    return 0;
}
...
static vm_fault_t do_fault_around(struct vm_fault *vmf)
{
	 ...
	 */
    end_pgoff = start_pgoff - ((vmf->address >> PAGE_SHIFT) & (PTRS_PER_PTE - 1)) + PTRS_PER_PTE - 1;
    end_pgoff = min3(end_pgoff, vma_pages(vmf->vma) + vmf->vma->vm_pgoff - 1, start_pgoff + nr_pages - 1);
...
static int numa_migrate_prep(struct page *page, struct vm_area_struct *vma, unsigned long addr, int page_nid,
                             int *flags)
{
    get_page(page);
...
/*
 * By the time we get here, we already hold the mm semaphore 已经 down 了 mm 的 信号量
 *
 * The mmap_lock may have been released depending on flags and our
 * return value.  See filemap_fault() and __lock_page_or_retry().
 *
...
static vm_fault_t __handle_mm_fault(struct vm_area_struct *vma, unsigned long address, unsigned int flags)
{
    ...
    pgd = pgd_offset(mm, address); /* 页全局目录 项 */
    p4d = p4d_alloc(mm, pgd, address); /* 页四级目录 项 */
    if (!p4d)
        return VM_FAULT_OOM;
...
retry_pud:
    ...
    /**
	 * 如果 pud 项 为空，并且使能了 透明大页内存，那么可以断定，这是一个透明大页
	 */
    ...
    /**
	 * 如果 pmd 项 为空，并且使能了 透明大页内存，那么可以断定，这是一个透明大页
	 */
 ...
 */
static inline void mm_account_fault(struct pt_regs *regs, unsigned long address, unsigned int flags, vm_fault_t ret)
{
    bool major;
...
vm_fault_t handle_mm_fault(struct vm_area_struct *vma, unsigned long address, unsigned int flags, struct pt_regs *regs)
{
	 ...
	 * 没有访问权限，直接段错误
	 */
    if (!arch_vma_access_permitted(vma, flags & FAULT_FLAG_WRITE, flags & FAULT_FLAG_INSTRUCTION,
...
static int __follow_pte_pmd(struct mm_struct *mm, unsigned long address, struct mmu_notifier_range *range,
                            pte_t **ptepp, pmd_t **pmdpp, spinlock_t **ptlp)
{
    ...
    if (pmd_huge(*pmd)) {
        ...
        if (range) {
            mmu_notifier_range_init(range, MMU_NOTIFY_CLEAR, 0, NULL, mm, address & PMD_MASK,
    ...
    if (range) {
        mmu_notifier_range_init(range, MMU_NOTIFY_CLEAR, 0, NULL, mm, address & PAGE_MASK,
...
static inline int follow_pte(struct mm_struct *mm, unsigned long address, pte_t **ptepp, spinlock_t **ptlp)
{
    ...
    (void)__cond_lock(*ptlp, !(res = __follow_pte_pmd(mm, address, NULL, ptepp, NULL, ptlp)));
    return res;
}

int follow_pte_pmd(struct mm_struct *mm, unsigned long address, struct mmu_notifier_range *range, pte_t **ptepp,
                   pmd_t **pmdpp, spinlock_t **ptlp)
{
    int res;

    /* (void) is needed to make gcc happy */
    (void)__cond_lock(*ptlp, !(res = __follow_pte_pmd(mm, address, range, ptepp, pmdpp, ptlp)));
    return res;
}
 ...
 */
int __access_remote_vm(struct task_struct *tsk, struct mm_struct *mm, unsigned long addr, void *buf, int len,
                       unsigned int gup_flags)
{
    ...
    while (len) {
        ...
        ret = get_user_pages_remote(mm, addr, 1, gup_flags, &page, &vma, NULL);
        if (ret <= 0) {
...
void print_vma_addr(char *prefix, unsigned long ip)
{
    ...
    if (vma && vma->vm_file) {
        ...
        if (buf) {
            ...
            if (IS_ERR(p))
                p = "?";
            printk("%s%s[%lx+%lx]", prefix, kbasename(p), vma->vm_start, vma->vm_end - vma->vm_start);
 ...
 * cache lines hot.
 */
static inline void process_huge_page(unsigned long addr_hint, unsigned int pages_per_huge_page,
                                     void (*process_subpage)(unsigned long addr, int idx, void *arg), void *arg)
{
    int i, n, base, l;
    unsigned long addr = addr_hint & ~(((unsigned long)pages_per_huge_page << PAGE_SHIFT) - 1);
...
static void clear_gigantic_page(struct page *page, unsigned long addr, unsigned int pages_per_huge_page)
{
    ...
    might_sleep();
    for (i = 0; i < pages_per_huge_page; i++, p = mem_map_next(p, page, i)) {
        cond_resched();
...
static void copy_user_gigantic_page(struct page *dst, struct page *src, unsigned long addr, struct vm_area_struct *vma,
...
static void copy_subpage(unsigned long addr, int idx, void *arg)
{
    ...
    copy_user_highpage(copy_arg->dst + idx, copy_arg->src + idx, addr, copy_arg->vma);
}

void copy_user_huge_page(struct page *dst, struct page *src, unsigned long addr_hint, struct vm_area_struct *vma,
                         unsigned int pages_per_huge_page)
{
    unsigned long addr = addr_hint & ~(((unsigned long)pages_per_huge_page << PAGE_SHIFT) - 1);
...
long copy_huge_page_from_user(struct page *dst_page, const void __user *usr_src, unsigned int pages_per_huge_page,
                              bool allow_pagefault)
{
    ...
    for (i = 0; i < pages_per_huge_page; i++) {
        ...
        rc = copy_from_user(page_kaddr, (const void __user *)(src + i * PAGE_SIZE), PAGE_SIZE);
        if (allow_pagefault)
            kunmap(dst_page + i);
        else
...
void __init ptlock_cache_init(void) /* 页表的自旋锁分配内存 */
{
    page_ptl_cachep = kmem_cache_create("page->ptl", sizeof(spinlock_t), 0, SLAB_PANIC, NULL);
}
