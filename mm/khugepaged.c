...
static struct task_struct __read_mostly *khugepaged_thread;
...
static unsigned int __read_mostly khugepaged_scan_sleep_millisecs = 10000;
/* during fragmentation poll the hugepage allocator once every minute */
static unsigned int __read_mostly khugepaged_alloc_sleep_millisecs = 60000;
 ...
 * fault.
 */
static unsigned int __read_mostly khugepaged_max_ptes_none;
static unsigned int __read_mostly khugepaged_max_ptes_swap;
static unsigned int __read_mostly khugepaged_max_ptes_shared;

#define MM_SLOTS_HASH_BITS 10
static __read_mostly DEFINE_HASHTABLE(mm_slots_hash, MM_SLOTS_HASH_BITS);

static struct kmem_cache __read_mostly *mm_slot_cache;
...
static ssize_t scan_sleep_millisecs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                          size_t count)
{
    unsigned long msecs;
    int err;
...
static struct kobj_attribute scan_sleep_millisecs_attr =
        __ATTR(scan_sleep_millisecs, 0644, scan_sleep_millisecs_show, scan_sleep_millisecs_store);
...
static ssize_t alloc_sleep_millisecs_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                           size_t count)
{
    unsigned long msecs;
    int err;
...
static struct kobj_attribute alloc_sleep_millisecs_attr =
        __ATTR(alloc_sleep_millisecs, 0644, alloc_sleep_millisecs_show, alloc_sleep_millisecs_store);
...
static struct kobj_attribute pages_to_scan_attr = __ATTR(pages_to_scan, 0644, pages_to_scan_show, pages_to_scan_store);

static ssize_t pages_collapsed_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", khugepaged_pages_collapsed);
}
static struct kobj_attribute pages_collapsed_attr = __ATTR_RO(pages_collapsed);

static ssize_t full_scans_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", khugepaged_full_scans);
}
static struct kobj_attribute full_scans_attr = __ATTR_RO(full_scans);
...
{
    return single_hugepage_flag_store(kobj, attr, buf, count, TRANSPARENT_HUGEPAGE_DEFRAG_KHUGEPAGED_FLAG);
}
static struct kobj_attribute khugepaged_defrag_attr =
        __ATTR(defrag, 0644, khugepaged_defrag_show, khugepaged_defrag_store);
...
static ssize_t khugepaged_max_ptes_none_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                              size_t count)
{
    int err;
...
static struct kobj_attribute khugepaged_max_ptes_none_attr =
        __ATTR(max_ptes_none, 0644, khugepaged_max_ptes_none_show, khugepaged_max_ptes_none_store);
...
static ssize_t khugepaged_max_ptes_swap_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                              size_t count)
{
    int err;
...
static struct kobj_attribute khugepaged_max_ptes_swap_attr =
        __ATTR(max_ptes_swap, 0644, khugepaged_max_ptes_swap_show, khugepaged_max_ptes_swap_store);

static ssize_t khugepaged_max_ptes_shared_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", khugepaged_max_ptes_shared);
}

static ssize_t khugepaged_max_ptes_shared_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
                                                size_t count)
{
    int err;
...
static struct kobj_attribute khugepaged_max_ptes_shared_attr =
        __ATTR(max_ptes_shared, 0644, khugepaged_max_ptes_shared_show, khugepaged_max_ptes_shared_store);
...
int hugepage_madvise(struct vm_area_struct *vma, unsigned long *vm_flags, int advice)
{
    switch (advice) {
		 ...
		 */
            if (!(*vm_flags & VM_NO_KHUGEPAGED) && khugepaged_enter_vma_merge(vma, *vm_flags))
                return -ENOMEM;
            break;
...
{
    mm_slot_cache =
            kmem_cache_create("khugepaged_mm_slot", sizeof(struct mm_slot), __alignof__(struct mm_slot), 0, NULL);
    if (!mm_slot_cache)
        return -ENOMEM;
...
static bool hugepage_vma_check(struct vm_area_struct *vma, unsigned long vm_flags)
{
    ...
    if (shmem_file(vma->vm_file) ||
        (IS_ENABLED(CONFIG_READ_ONLY_THP_FOR_FS) && vma->vm_file && (vm_flags & VM_DENYWRITE))) {
        return IS_ALIGNED((vma->vm_start >> PAGE_SHIFT) - vma->vm_pgoff, HPAGE_PMD_NR);
    }
...
static void release_pte_pages(pte_t *pte, pte_t *_pte, struct list_head *compound_pagelist)
{
    ...
    while (--_pte >= pte) {
        ...
        if (!pte_none(pteval) && !is_zero_pfn(pte_pfn(pteval)) && !PageCompound(page))
                                        ...
                                        struct list_head *compound_pagelist)
{
    ...
    for (_pte = pte; _pte < pte + HPAGE_PMD_NR; _pte++, address += PAGE_SIZE) {
        pte_t pteval = *_pte;
        if (pte_none(pteval) || (pte_present(pteval) && is_zero_pfn(pte_pfn(pteval)))) {
    ...
    if (likely(writable)) {
        if (likely(referenced)) {
            result = SCAN_SUCCEED;
            trace_mm_collapse_huge_page_isolate(page, none_or_zero, referenced, writable, result);
            return 1;
        }
    } else {
        result = SCAN_PAGE_RO;
...
static void __collapse_huge_page_copy(pte_t *pte, struct page *page, struct vm_area_struct *vma, unsigned long address,
...
static void khugepaged_alloc_sleep(void)
{
    ...
    freezable_schedule_timeout_interruptible(msecs_to_jiffies(khugepaged_alloc_sleep_millisecs));
...
static int khugepaged_find_target_node(void)
{
    ...
    if (target_node <= last_khugepaged_target_node)
        for (nid = last_khugepaged_target_node + 1; nid < MAX_NUMNODES; nid++)
...
static bool __collapse_huge_page_swapin(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long address,
                                        pmd_t *pmd, int referenced)
{
    ...
    for (; vmf.address < address + HPAGE_PMD_NR * PAGE_SIZE; vmf.pte++, vmf.address += PAGE_SIZE) {
                               ...
                               int referenced, int unmapped)
{
    ...
    mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, NULL, mm, address, address + HPAGE_PMD_SIZE);
 ...
 */
static int khugepaged_add_pte_mapped_thp(struct mm_struct *mm, unsigned long addr)
{
    struct mm_slot *mm_slot;
...
void collapse_pte_mapped_thp(struct mm_struct *mm, unsigned long addr)
{
    ...
    if (!vma || !vma->vm_file || vma->vm_start > haddr || vma->vm_end < haddr + HPAGE_PMD_SIZE)
        return;
    ...
    hpage = find_lock_page(vma->vm_file->f_mapping, linear_page_index(vma, haddr));
    if (!hpage)
        return;
    ...
    for (i = 0, addr = haddr, pte = start_pte; i < HPAGE_PMD_NR; i++, addr += PAGE_SIZE, pte++) {
        struct page *page;
 ...
 */
static void collapse_file(struct mm_struct *mm, struct file *file, pgoff_t start, struct page **hpage, int node)
{
    ...
    for (index = start; index < end; index++) {
        ...
        if (is_shmem) {
            ...
            if (xa_is_value(page) || !PageUptodate(page)) {
                ...
                if (shmem_getpage(mapping->host, index, &page, SGP_NOHUGE)) {
                    result = SCAN_FAIL;
                    goto xa_unlocked;
                }
...
xa_unlocked:
    ...
    if (result == SCAN_SUCCEED) {
        ...
        list_for_each_entry_safe(page, tmp, &pagelist, lru)
        {
            ...
            copy_highpage(new_page + (page->index % HPAGE_PMD_NR), page);
            list_del(&page->lru);
            page->mapping = NULL;
...
/**
 * 扫描指定内存槽中的虚拟内存区域(VMA)，寻找可以合并成大页的连续页面
 *
 ...
 * khugepaged 4096
 */
static unsigned int khugepaged_scan_mm_slot(unsigned int pages, struct page **hpage) __releases(&khugepaged_mm_lock)
        __acquires(&khugepaged_mm_lock)
{
    ...
    /* 更新扫描状态并收集完成的mm_slot */
    spin_lock(&khugepaged_mm_lock);
    if (!vma || khugepaged_test_exit(mm)) {
        /* 完成扫描,更新下一个扫描位置 */
        if (mm_slot->mm_node.next != &khugepaged_scan.mm_head) {
            khugepaged_scan.mm_slot = list_entry(mm_slot->mm_node.next, struct mm_slot, mm_node);
            khugepaged_scan.address = 0;
        } else {
...
{
    return !list_empty(&khugepaged_scan.mm_head) && khugepaged_enabled();
}

static int khugepaged_wait_event(void)
{
    return !list_empty(&khugepaged_scan.mm_head) || kthread_should_stop();
}
...
static void khugepaged_do_scan(void)
{
    ...
    /* 确保所有CPU的LRU缓存都已刷新到全局LRU列表 */
    lru_add_drain_all();

    /* 主扫描循环 - 直到达到扫描页面数量限制 */
    while (progress < pages) {
        /* 预分配一个hugepage，如果分配失败则退出循环 */
        ...
        /**
        * 如果还有工作要做且没有完全遍历链表两次：
        * - 调用khugepaged_scan_mm_slot()扫描当前内存槽
        * - khugepaged_scan_mm_slot会处理页面合并
        * - 返回处理的页面数量
        */
...
{
    return kthread_should_stop() || time_after_eq(jiffies, khugepaged_sleep_expire);
}
...
static void khugepaged_wait_work(void)
{
    if (khugepaged_has_work()) {
        const unsigned long scan_sleep_jiffies = msecs_to_jiffies(khugepaged_scan_sleep_millisecs);

        if (!scan_sleep_jiffies)
            return;

        khugepaged_sleep_expire = jiffies + scan_sleep_jiffies;
        wait_event_freezable_timeout(khugepaged_wait, khugepaged_should_wakeup(), scan_sleep_jiffies);
        return;
    }
...
static int khugepaged(void *none)
{
    ...
    while (!kthread_should_stop()) {
        ...
        /*
        * 等待下一次扫描
        * - 如果有工作要做，会等待khugepaged_scan_sleep_millisecs
        * - 如果没有工作，会等待直到新的任务到来
        */
...
static void set_recommended_min_free_kbytes(void)
{
    ...
    recommended_min = min(recommended_min, (unsigned long)nr_free_buffer_pages() / 20);
