 ...
 */
#if MAX_NUMNODES <= 256
//static u8 __cacheline_aligned section_to_node_table[NR_MEM_SECTIONS] ;
#else
static u16 __cacheline_aligned section_to_node_table[NR_MEM_SECTIONS];
#endif
...
#define for_each_present_section_nr(start, section_nr)                         \
    for (section_nr = next_present_section_nr(start - 1);                      \
         ((section_nr != -1) && (section_nr <= __highest_present_section_nr)); \
...
static void subsection_mask_set(unsigned long *map, unsigned long pfn, unsigned long nr_pages)
{
...
void __init subsection_map_init(unsigned long pfn, unsigned long nr_pages)
{
    ...
    for (nr = start_sec; nr <= end_sec; nr++) {
        ...
        pr_debug("%s: sec: %lu pfns: %lu set(%d, %d)\n", __func__, nr, pfns, subsection_map_index(pfn),
...
static void __meminit sparse_init_one_section(struct mem_section *ms, unsigned long pnum, struct page *mem_map,
...
static struct mem_section_usage *__init sparse_early_usemaps_alloc_pgdat_section(struct pglist_data *pgdat,
                                                                                 unsigned long size)
{
...
static void __init check_usemap_section_nr(int nid, struct mem_section_usage *usage)
{
...
static void __meminitdata *sparsemap_buf_end;
...
static void __init sparse_buffer_init(unsigned long size, int nid)
{
	 ...
	 */
    sparsemap_buf = memblock_alloc_exact_nid_raw(size, section_map_size(), addr, MEMBLOCK_ALLOC_ACCESSIBLE, nid);
 ...
 *
 * 该函数是稀疏内存模型初始化的入口点。稀疏内存模型用于管理物理内存,
 * 特别适用于具有大型内存空洞的系统。它的主要任务包括:
 * 
 * 1. 标记所有可用的内存块
 * 2. 为每个NUMA节点初始化内存段(sections)
 * 3. 设置内存映射(memmap)和使用位图(usemap)
 * 4. 准备支持内存热插拔
 *
 * 这个初始化过程是按NUMA节点进行的,以优化NUMA系统的性能。
 * 它还为后续的内存管理操作(如内存热插拔)奠定了基础。
 */
...
#ifdef CONFIG_SPARSEMEM_VMEMMAP
static struct page *__meminit populate_section_memmap(unsigned long pfn, unsigned long nr_pages, int nid,
                                                      struct vmem_altmap *altmap)
{
    return __populate_section_memmap(pfn, nr_pages, nid, altmap);
}

static void depopulate_section_memmap(unsigned long pfn, unsigned long nr_pages, struct vmem_altmap *altmap)
{
...
static int clear_subsection_map(unsigned long pfn, unsigned long nr_pages)
{
    ...
    unsigned long *subsection_map = ms->usage ? &ms->usage->subsection_map[0] : NULL;
...
{
    return bitmap_empty(&ms->usage->subsection_map[0], SUBSECTIONS_PER_SECTION);
}
...
static int fill_subsection_map(unsigned long pfn, unsigned long nr_pages)
{
    ...
    if (bitmap_empty(map, SUBSECTIONS_PER_SECTION))
        ...
        bitmap_or(subsection_map, map, subsection_map, SUBSECTIONS_PER_SECTION);
 ...
 */
static void section_deactivate(unsigned long pfn, unsigned long nr_pages, struct vmem_altmap *altmap)
{
    struct mem_section *ms = __pfn_to_section(pfn);
...
static struct page *__meminit section_activate(int nid, unsigned long pfn, unsigned long nr_pages,
                                               struct vmem_altmap *altmap)
{
    struct mem_section *ms = __pfn_to_section(pfn);
 ...
 *
 */
void sparse_remove_section(struct mem_section *ms, unsigned long pfn, unsigned long nr_pages, unsigned long map_offset,
                           struct vmem_altmap *altmap)
{
